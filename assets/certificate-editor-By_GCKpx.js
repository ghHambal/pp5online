import{d as xe,U as ye,v as V,w as de,x as fe}from"./certificate-engine-CjKzMmMi.js";import{uploadCertificateTemplateImage as me,uploadCertificateLogoImage as le}from"./storage-CuUjCgvI.js";import{a as B,g as Z}from"./ui-DI1UEpN2.js";const m=S=>String(S??"").replace(/[&<>"']/g,u=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[u]),f=(S,u,X)=>Math.min(X,Math.max(u,S)),H=S=>`${S}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`,ce=S=>String(S||"").replace(/[^A-Za-z0-9 _-]/g,"").trim();function W(S){const u=ce(S);if(!u)return;const X=`cert-font-${u.toLowerCase().replace(/\s+/g,"-")}`;if(document.getElementById(X))return;const z=document.createElement("link");z.id=X,z.rel="stylesheet",z.href=`https://fonts.googleapis.com/css2?family=${encodeURIComponent(u).replace(/%20/g,"+")}:wght@400;600;700&display=swap`,document.head.appendChild(z)}function ke(S){var ae,ne,ie;const{template:u,previewVariables:X,placeholderTokens:z=[],onSave:se}=S;(ae=document.getElementById("cce-overlay"))==null||ae.remove();let c=JSON.parse(JSON.stringify((ne=u==null?void 0:u.layout)!=null&&ne.elements?u.layout:xe((u==null?void 0:u.type)==="custom"?"custom":(u==null?void 0:u.preset_key)??"gold_classic")));((ie=c.background)==null?void 0:ie.type)==="image"&&!c.background.imageUrl&&(u!=null&&u.background_image_url)&&(c.background.imageUrl=u.background_image_url),c.orientation||(c.orientation="landscape"),Array.isArray(c.customFonts)||(c.customFonts=[]);let p=new Set,Y=null;const Q=new Map;[...ye,...z].forEach(e=>Q.set(e.token,e));const ue=[...Q.values()],be={name:"เด็กชาย ตัวอย่าง นักเรียน",student_code:"25944",date:new Date().toLocaleDateString("th-TH",{dateStyle:"long"}),no:"CERT-2569-000001",...X};[...V,...c.customFonts,...(c.elements??[]).map(e=>e.fontFamily)].filter(Boolean).forEach(W);const pe=document.body.style.overflow;document.body.style.overflow="hidden";const g=document.createElement("div");g.id="cce-overlay",g.className="fixed inset-0 z-[9999] bg-white flex flex-col",g.innerHTML=`
    ...
    </div>`,document.body.appendChild(g);const G=g.querySelector("#cce-canvas-wrap"),i=g.querySelector("#cce-panel"),ee=g.querySelector("#cce-duplicate"),q=e=>`<p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">${e}</p>`,v=({id:e,label:t,value:a,min:d,max:r,step:l=1,suffix:s=""})=>`
