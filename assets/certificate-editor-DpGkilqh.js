import{d as xe,U as ye,h as V,j as de,r as fe}from"./certificate-engine-BrPYUHds.js";import{uploadCertificateTemplateImage as me,uploadCertificateLogoImage as le}from"./storage-CuUjCgvI.js";import{a as A,g as Z}from"./ui-MMtcTwtt.js";const m=S=>String(S??"").replace(/[&<>"']/g,u=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[u]),f=(S,u,X)=>Math.min(X,Math.max(u,S)),H=S=>`${S}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`,ce=S=>String(S||"").replace(/[^A-Za-z0-9 _-]/g,"").trim();function W(S){const u=ce(S);if(!u)return;const X=`cert-font-${u.toLowerCase().replace(/\s+/g,"-")}`;if(document.getElementById(X))return;const z=document.createElement("link");z.id=X,z.rel="stylesheet",z.href=`https://fonts.googleapis.com/css2?family=${encodeURIComponent(u).replace(/%20/g,"+")}:wght@400;600;700&display=swap`,document.head.appendChild(z)}function ke(S){var ae,ne,ie;const{template:u,previewVariables:X,placeholderTokens:z=[],onSave:se}=S;(ae=document.getElementById("cce-overlay"))==null||ae.remove();let c=JSON.parse(JSON.stringify((ne=u==null?void 0:u.layout)!=null&&ne.elements?u.layout:xe((u==null?void 0:u.type)==="custom"?"custom":(u==null?void 0:u.preset_key)??"gold_classic")));((ie=c.background)==null?void 0:ie.type)==="image"&&!c.background.imageUrl&&(u!=null&&u.background_image_url)&&(c.background.imageUrl=u.background_image_url),c.orientation||(c.orientation="landscape"),Array.isArray(c.customFonts)||(c.customFonts=[]);let p=new Set,Y=null;const Q=new Map;[...ye,...z].forEach(e=>Q.set(e.token,e));const ue=[...Q.values()],be={name:"เด็กชาย ตัวอย่าง นักเรียน",student_code:"25944",date:new Date().toLocaleDateString("th-TH",{dateStyle:"long"}),no:"CERT-2569-000001",...X};[...V,...c.customFonts,...(c.elements??[]).map(e=>e.fontFamily)].filter(Boolean).forEach(W);const pe=document.body.style.overflow;document.body.style.overflow="hidden";const g=document.createElement("div");g.id="cce-overlay",g.className="fixed inset-0 z-[9999] bg-white flex flex-col",g.innerHTML=`
        ...
        <div class="max-w-3xl w-full mx-auto grid sm:grid-cols-2 gap-3">
          ...
          <div class="bg-white border border-gray-200 rounded-xl p-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              ${Object.entries(de).map(([e,t])=>`<button type="button" class="cce-bg-preset w-7 h-7 rounded-full border-2 border-white shadow" style="background:${t.bg}" data-preset="${e}" title="${m(e)}"></button>`).join("")}
            </div>
          </div>
        </div>
      </div>
    ...
    </div>`,document.body.appendChild(g);const B=g.querySelector("#cce-canvas-wrap"),i=g.querySelector("#cce-panel"),ee=g.querySelector("#cce-duplicate"),q=e=>`<p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">${e}</p>`,v=({id:e,label:t,value:a,min:d,max:r,step:l=1,suffix:s=""})=>`
    <label class="block text-[11px] text-gray-500">
    ...
    </label>`,h=(e,t,a,d="")=>{const r=e.querySelector(`#${t}`);if(!r)return;const l=e.querySelector(`[data-range-value="${t}"]`);r.addEventListener("input",s=>{l&&(l.textContent=`${s.target.value}${d}`),a(s)})};function te(){return(c.elements??[]).filter(e=>p.has(e.id))}function y(){const e=c.orientation==="portrait";B.style.maxWidth=e?"26rem":"48rem",B.innerHTML=fe({layout:c,variables:be});const t=B.querySelector(".cert-canvas");t==null||t.addEventListener("pointerdown",r=>{if(r.target.closest("[data-cert-el-id]"))return;r.preventDefault();const l=r.shiftKey||r.ctrlKey||r.metaKey,s=l?new Set(p):new Set,o=t.getBoundingClientRect(),b=r.clientX,w=r.clientY,F=[...t.querySelectorAll("[data-cert-el-id]")].map(n=>({id:n.dataset.certElId,rect:n.getBoundingClientRect()}));let N=!1;const k=document.createElement("div");k.style.cssText="position:absolute;border:1.5px dashed #6366f1;background:rgba(99,102,241,0.12);pointer-events:none;z-index:60;display:none;",t.appendChild(k);const x=n=>{const M=n.clientX-b,O=n.clientY-w;if(!N&&Math.hypot(M,O)<4)return;N=!0;const j=Math.min(b,n.clientX),D=Math.max(b,n.clientX),U=Math.min(w,n.clientY),T=Math.max(w,n.clientY);k.style.display="block",k.style.left=j-o.left+"px",k.style.top=U-o.top+"px",k.style.width=D-j+"px",k.style.height=T-U+"px";const P=F.filter($=>$.rect.left<D&&$.rect.right>j&&$.rect.top<T&&$.rect.bottom>U).map($=>$.id);p=new Set([...s,...P]),t.querySelectorAll("[data-cert-el-id]").forEach($=>{$.style.outline=p.has($.dataset.certElId)?"2px dashed #0ea5e9":"none"})},L=()=>{window.removeEventListener("pointermove",x),window.removeEventListener("pointerup",L),k.remove(),!N&&!l&&p.clear(),y(),E()};window.addEventListener("pointermove",x),window.addEventListener("pointerup",L)}),B.querySelectorAll("[data-cert-el-id]").forEach(r=>{const l=r.dataset.certElId,s=c.elements.find(o=>o.id===l);r.style.cursor=(s==null?void 0:s.type)==="cornerGraphic"?"pointer":"move",r.style.outline=p.has(l)?"2px dashed #0ea5e9":"none",r.style.outlineOffset="3px",r.addEventListener("pointerdown",o=>{if(o.shiftKey||o.ctrlKey||o.metaKey){o.preventDefault(),p.has(l)?p.delete(l):p.add(l),y(),E();return}if((s==null?void 0:s.type)==="cornerGraphic"){o.preventDefault(),p=new Set([l]),y(),E();return}ge(o,l)})}),ee.disabled=!p.size;const a="bg-indigo-600 text-white border-indigo-600",d="bg-white text-gray-700 border-gray-300";g.querySelector("#cce-orient-landscape").className=`cce-orient-btn flex-1 px-2.5 py-1.5 rounded-lg border text-xs font-bold ${e?d:a}`,g.querySelector("#cce-orient-portrait").className=`cce-orient-btn flex-1 px-2.5 py-1.5 rounded-lg border text-xs font-bold ${e?a:d}`}function _(){if(!p.size)return;const e=p.size;c.elements=c.elements.filter(t=>!p.has(t.id)),p.clear(),y(),E(),e>1&&A(`ลบ ${e} รายการแล้ว`,"success")}function G(){const e=te();if(!e.length)return;const t=e.map(a=>{const d=JSON.parse(JSON.stringify(a));return d.id=H(a.type==="image"||a.type==="cornerGraphic"?"img":"el"),d.x!=null&&(d.x=f(Number(d.x)+3,0,100)),d.y!=null&&(d.y=f(Number(d.y)+3,0,100)),d.type==="cornerGraphic"&&(d.insetY=f(Number(d.insetY||2)+3,0,45)),d});c.elements.push(...t),p=new Set(t.map(a=>a.id)),y(),E(),A(t.length>1?`คัดลอก ${t.length} รายการพร้อมรูปแบบและเอฟเฟกต์แล้ว`:"คัดลอกพร้อมรูปแบบและเอฟเฟกต์แล้ว","success")}function R(e){var d;const t=e.type==="cornerGraphic";i.innerHTML=`
      <div class="space-y-5">
        <div>
      ...
      <div class="space-y-5">
        <div>
          ...
          <div class="grid grid-cols-2 gap-1.5 mt-2">
            ${ue.map(o=>`<button type="button" title="แทรก ${m(o.token)}" class="cce-insert-token flex items-center gap-1.5 px-2.5 py-2 rounded-lg bg-indigo-50 text-indigo-700 text-[10px] font-bold hover:bg-indigo-100 text-left" data-token="${m(o.token)}"><span>⚡</span><span class="truncate">${m(o.label)}</span></button>`).join("")}
          </div>
        </div>
        <div class="pt-4 border-t border-gray-200">
          ${q("ฟอนต์")}
      ...
      <div class="space-y-5">
        ...
        <div>
          ...
          <div class="flex gap-1.5 mt-2.5 mb-3">
            ...
            ${[["left","≡","ชิดซ้าย"],["center","≣","กึ่งกลาง"],["right","≡","ชิดขวา"]].map(([n,M,O])=>`<button type="button" data-align="${n}" title="${O}" class="cce-bulk-align h-9 w-9 rounded-lg border font-bold ${a.align===n?"bg-indigo-600 text-white border-indigo-600":"border-gray-300 text-gray-600"}" style="${n==="right"?"transform:scaleX(-1)":""}">${M}</button>`).join("")}
          </div>
      ...
      </div>`;const r=n=>{e.forEach(M=>Object.assign(M,n)),y()},l=n=>{t.forEach(M=>Object.assign(M,n)),y()};i.querySelector("#cce-bulk-clear").addEventListener("click",()=>{p.clear(),y(),E()}),(F=i.querySelector("#cce-bulk-font"))==null||F.addEventListener("change",n=>{n.target.value&&(W(n.target.value),l({fontFamily:n.target.value}))}),(N=i.querySelector("#cce-bulk-bold"))==null||N.addEventListener("click",()=>{l({bold:!a.bold}),I(e)}),i.querySelectorAll(".cce-bulk-align").forEach(n=>n.addEventListener("click",()=>{l({align:n.dataset.align}),I(e)})),(k=i.querySelector("#cce-bulk-color"))==null||k.addEventListener("input",n=>l({color:n.target.value})),h(i,"cce-bulk-size",n=>l({fontSize:f(Number(n.target.value)||12,6,120)})," px"),h(i,"cce-bulk-letter",n=>l({letterSpacing:f(Number(n.target.value)||0,-5,20)})," px"),(x=i.querySelector("#cce-bulk-shadow"))==null||x.addEventListener("click",()=>{var n;l({shadow:{...a.shadow??{},enabled:!((n=a.shadow)!=null&&n.enabled)}}),I(e)}),(L=i.querySelector("#cce-bulk-stroke"))==null||L.addEventListener("click",()=>{var n;l({stroke:{...a.stroke??{},enabled:!((n=a.stroke)!=null&&n.enabled)}}),I(e)}),h(i,"cce-bulk-opacity",n=>r({opacity:f(Number(n.target.value)||0,0,100)/100}),"%"),i.querySelector("#cce-bulk-duplicate").addEventListener("click",G),i.querySelector("#cce-bulk-delete").addEventListener("click",_)}const oe=1.2;function ge(e,t){e.preventDefault();const a=p.has(t)&&p.size>1;a||(p=new Set([t])),y(),E();const d=B.querySelector(".cert-canvas"),r=d.getBoundingClientRect(),l=c.elements.find(x=>x.id===t),s=a?c.elements.filter(x=>p.has(x.id)&&x.x!=null&&x.y!=null):[l],o=new Map(s.map(x=>[x.id,{x:x.x,y:x.y}])),b=new Map(s.map(x=>[x.id,d.querySelector(`[data-cert-el-id="${x.id}"]`)])),w=document.createElement("div");w.style.cssText="position:absolute;top:0;bottom:0;left:50%;border-left:1.5px dashed #ec4899;pointer-events:none;z-index:50;display:none;";const F=document.createElement("div");F.style.cssText="position:absolute;left:0;right:0;top:50%;border-top:1.5px dashed #ec4899;pointer-events:none;z-index:50;display:none;",d.append(w,F);const N=x=>{let L=f((x.clientX-r.left)/r.width*100,0,100),n=f((x.clientY-r.top)/r.height*100,0,100);const M=Math.abs(L-50)<oe,O=Math.abs(n-50)<oe;M&&(L=50),O&&(n=50),w.style.display=M?"block":"none",F.style.display=O?"block":"none",L=Math.round(L*10)/10,n=Math.round(n*10)/10;const j=o.get(l.id),D=L-j.x,U=n-j.y;s.forEach(T=>{const P=o.get(T.id);T.x=f(Math.round((P.x+D)*10)/10,0,100),T.y=f(Math.round((P.y+U)*10)/10,0,100);const $=b.get(T.id);$&&($.style.left=T.x+"%",$.style.top=T.y+"%")})},k=()=>{window.removeEventListener("pointermove",N),window.removeEventListener("pointerup",k),y()};window.addEventListener("pointermove",N),window.addEventListener("pointerup",k)}async function K(e,t=null){if(e)try{const a=await le(e),d=t?{id:H("corner"),type:"cornerGraphic",imageUrl:a,position:t,width:14,insetX:2,insetY:2,opacity:1}:{id:H("img"),type:"image",imageUrl:a,x:50,y:15,width:15,opacity:1};c.elements.push(d),p=new Set([d.id]),y(),E()}catch(a){A("อัปโหลดไม่สำเร็จ: "+Z(a),"error")}}g.querySelector("#cce-add-el").addEventListener("click",()=>{const e={id:H("el"),text:"ข้อความใหม่",x:50,y:50,fontSize:16,color:"#1d1519",fontFamily:"Sarabun",align:"center",bold:!1,opacity:1};c.elements.push(e),p=new Set([e.id]),y(),E()}),g.querySelector("#cce-select-all").addEventListener("click",()=>{p=new Set(c.elements.map(e=>e.id)),y(),E()}),g.querySelector("#cce-add-logo-file").addEventListener("change",e=>{var t;K((t=e.target.files)==null?void 0:t[0]),e.target.value=""}),g.querySelector("#cce-add-corner-top").addEventListener("change",e=>{var t;K((t=e.target.files)==null?void 0:t[0],"top"),e.target.value=""}),g.querySelector("#cce-add-corner-bottom").addEventListener("change",e=>{var t;K((t=e.target.files)==null?void 0:t[0],"bottom"),e.target.value=""}),ee.addEventListener("click",G),g.querySelector("#cce-orient-landscape").addEventListener("click",()=>{c.orientation="landscape",y()}),g.querySelector("#cce-orient-portrait").addEventListener("click",()=>{c.orientation="portrait",y()}),g.querySelector("#cce-bg-file").addEventListener("change",e=>{var a;const t=(a=e.target.files)==null?void 0:a[0];t&&(Y=t,c.background={type:"image",imageUrl:URL.createObjectURL(t)},y())}),g.querySelectorAll(".cce-bg-preset").forEach(e=>e.addEventListener("click",()=>{const t=de[e.dataset.preset];Y=null,c.background={type:"flat",color:t.bg,cardColor:t.cardBg,borderColor:t.border,borderWidth:t.borderWidth,borderStyle:t.borderStyle},y()}));const J=()=>{document.removeEventListener("keydown",re),document.body.style.overflow=pe,g.remove()},re=e=>{var a;const t=["INPUT","TEXTAREA","SELECT"].includes((a=e.target)==null?void 0:a.tagName);e.key==="Escape"&&J(),!t&&(e.key==="Delete"||e.key==="Backspace")&&_(),!t&&(e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="d"&&(e.preventDefault(),G())};document.addEventListener("keydown",re),g.querySelector("#cce-close").addEventListener("click",J),g.querySelector("#cce-save").addEventListener("click",async()=>{const e=g.querySelector("#cce-save");e.disabled=!0,e.textContent="กำลังบันทึก...";try{let t;Y&&(t=await me(Y)),t&&(c.background={type:"image",imageUrl:t}),await se(c,t),J()}catch(t){A("บันทึกไม่สำเร็จ: "+Z(t),"error"),e.disabled=!1,e.textContent="💾 บันทึก"}}),y(),E()}export{ke as o};
