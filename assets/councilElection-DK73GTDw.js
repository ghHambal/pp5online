import{s as h}from"./supabase-BV-W2lsh.js";/* empty css                                  */import{e as r,t as y}from"./sports-portals-KlTGN7Hq.js";import"./azizgames-modal-C0esVYMf.js";import"./version.js_v_10.22-mqYkeZoJ.js";import"./print-overlay-BVfxEd6n.js";import"./storage-CuUjCgvI.js";import"./impersonation-0xVfgYVY.js";import"./browser-JP79f-a9.js";const d=document.getElementById("council-election-app");function _(e){return`
  ...
  `;const e=d.querySelector("#student-code-input"),l=d.querySelector("#student-code-error"),s=d.querySelector("#student-confirm-card");e.focus();const m=async()=>{const i=e.value.trim();if(l.classList.add("hidden"),s.classList.add("hidden"),s.innerHTML="",!i)return;const{data:a,error:p}=await h.rpc("get_public_council_election_bundle",{p_code:i});if(p){l.textContent="เกิดข้อผิดพลาด กรุณาลองใหม่",l.classList.remove("hidden");return}if((a==null?void 0:a.error)==="student_not_found"){l.textContent="ไม่พบรหัสนักเรียนนี้ กรุณาตรวจสอบอีกครั้ง",l.classList.remove("hidden");return}if((a==null?void 0:a.error)==="gender_unknown"){l.textContent="ไม่พบข้อมูลเพศของนักเรียน ติดต่อผู้ดูแลระบบ",l.classList.remove("hidden");return}u(a,i)};e.addEventListener("blur",m),e.addEventListener("keydown",i=>{i.key==="Enter"&&(i.preventDefault(),e.blur())});function u(i,a){const p=i.student;s.innerHTML=`
      ${_(p)}
    ...
    `,s.classList.remove("hidden"),s.querySelector("#btn-enter-vote").addEventListener("click",()=>k(i,a)),s.querySelector("#btn-not-me").addEventListener("click",()=>{e.value="",s.classList.add("hidden"),e.focus()})}}function k(e,l){if(e.error==="election_not_found"){d.innerHTML=`
      <button id="btn-back" class="text-xs font-bold text-white/80 hover:text-white mb-3">← กลับไปกรอกรหัสใหม่</button>
      <div class="bg-white rounded-2xl border p-6 text-center text-[var(--muted-2)] text-sm">ยังไม่มีการเลือกตั้งสำหรับสภาของคุณในขณะนี้</div>`,d.querySelector("#btn-back").addEventListener("click",b);return}const s=e.already_voted_candidate_id,m=s?e.candidates.find(t=>t.id===s):null;if(s){d.innerHTML=`
    ...
    <div class="bg-white rounded-2xl border p-5 space-y-3">
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[85vh] overflow-y-auto">
        ...
        <div class="p-5 space-y-3">
          ...
          ${o.length?`<div><p class="text-xs font-bold text-[var(--muted)] mb-1">นโยบาย</p><ul class="text-sm text-[var(--ink-2)] list-disc list-inside space-y-0.5">${o.map(x=>`<li>${r(x)}</li>`).join("")}</ul></div>`:""}
          ...
          ${!t.slogan&&!t.vision&&!o.length&&!n.length?'<p class="text-xs text-[var(--muted-2)] text-center py-4">ยังไม่ได้กรอกข้อมูลโปรไฟล์เพิ่มเติม</p>':""}
      ...
      </div>`,document.body.appendChild(c);const v=()=>c.remove();c.addEventListener("click",x=>{x.target===c&&v()}),c.querySelector("#profile-modal-close").addEventListener("click",v),c.querySelector("#profile-modal-select").addEventListener("click",()=>{v(),u=t.id,f(),g()})},f=()=>{i.innerHTML=e.candidates.map(t=>`
      ...
      </div>`,a.querySelector("#btn-confirm-vote").addEventListener("click",async()=>{const o=a.querySelector("#btn-confirm-vote");o.disabled=!0,o.textContent="กำลังบันทึก...";const{data:n,error:c}=await h.rpc("cast_public_council_vote",{p_code:l,p_candidate_id:u});if(c||n!=null&&n.error){const v=(n==null?void 0:n.error)==="already_voted"?"รหัสนี้ลงคะแนนไปแล้ว":(n==null?void 0:n.error)==="election_not_open"?"ปิดโหวตแล้ว":"บันทึกไม่สำเร็จ กรุณาลองใหม่";y(v,"error"),o.disabled=!1,o.textContent="✅ ยืนยันลงคะแนน";return}y("บันทึกคะแนนแล้ว ขอบคุณที่ใช้สิทธิ์!"),e.already_voted_candidate_id=u,k(e,l)})};f()}b();
