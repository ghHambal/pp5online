import{getSystemConfig as Rt,getClassSessionDOWs as ut,getClassStudents as Ot,getClassAttendanceAll as Lt,getScoreColumns as Tt,getStudentScores as Et,getDepartments as Nt,getHomeroomTeachers as Pt,getTeacherById as Wt,getCourseDocPage2 as Bt,getCourseDocLangSettings as Ut,getSchoolHolidays as Vt,getLifeSkillColumns as Gt,getLifeSkillScores as It,getReadingScoreColumns as Ft,getReadingScores as qt}from"./api-Cf_Y4s92.js";import{a as dt,g as Yt}from"./ui-FQqAmrdo.js";import{s as gt}from"./supabase-BV-W2lsh.js";import{o as bt}from"./storage-D6nkcVz6.js";import{applyReadingGradesFromConfig as Kt,_readingGrade as Jt}from"./teacher-views-utils-BWmONzsh.js";function n(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function wt(t){if(!t)return null;const[i,s,c]=String(t).split("-").map(Number);return!i||!s||!c?null:new Date(i,s-1,c)}function st(t){return`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`}function $t(t){if(!t)return"";const[i,s,c]=String(t).split("-").map(Number);if(!i)return t;const o=(i+543)%100;return`${String(c).padStart(2,"0")}/${String(s).padStart(2,"0")}/${String(o).padStart(2,"0")}`}function Xt(t,i,s=null,c=!1){const o=c&&s&&s.length?s.length:Math.max(1,Math.round((i??1)*2)),e=o*20;let l=s&&s.length?[...s]:null,M=!1;if(l&&l.length<o){M=!0;const f={};for(const p of l)f[p]=(f[p]||0)+1;const m=["day1_date","day2_date","day3_date","day4_date","day5_date","day6_date"].map(p=>t[p]).filter(Boolean).map(p=>wt(p)).filter(Boolean).sort((p,r)=>p-r),a={};m.forEach(p=>{const r=p.getDay();a[r]=(a[r]||0)+1});const g=Object.entries(a).sort(([,p],[,r])=>r-p||Number(p)-Number(r));for(const[p]of g){if(l.length>=o)break;const r=Number(p);for(;(f[r]||0)<Math.min(a[r],2)&&l.length<o;)l.push(r),f[r]=(f[r]||0)+1}for(let p=1;p<=5&&l.length<o;p++)(f[p]||0)<2&&(l.push(p),f[p]=(f[p]||0)+1);l.sort((p,r)=>p-r)}else l&&l.length>o&&(l=l.slice(0,o));const v=["day1_date","day2_date","day3_date","day4_date","day5_date","day6_date"].map(f=>t[f]).filter(Boolean).map(f=>wt(f)).filter(Boolean).sort((f,m)=>f-m);if(!v.length)return[];if(M&&l){const m=[];let a=0,g=0;for(;a<v.length&&m.length<e;){const p=new Date(v[a]);p.setDate(p.getDate()-p.getDay()),p.setHours(0,0,0,0);const r=p.getTime();g=r;const $=[];for(;a<v.length&&v[a].getTime()>=r&&v[a].getTime()<r+6048e5;)$.push(v[a++]);const w={};for(const A of $){const S=A.getDay();w[S]=(w[S]||0)+1}for(const A of $){if(m.length>=e)break;m.push({n:m.length+1,date:new Date(A),ds:st(A)})}const j=o-$.length;if(j>0){const A={};l.forEach(R=>{A[R]=(A[R]||0)+1});const S=[];for(const[R,T]of Object.entries(A).sort()){const B=Number(R),P=w[B]||0;for(let O=0;O<T-P&&S.length<j;O++)S.push(B)}for(const R of S){if(m.length>=e)break;const T=new Date(p);T.setDate(T.getDate()+R),m.push({n:m.length+1,date:T,ds:st(T)})}}}if(m.length<e){const p=new Date(g);let r=1;for(;m.length<e;){for(const $ of l){if(m.length>=e)break;const w=new Date(p);w.setDate(w.getDate()+r*7+$),m.push({n:m.length+1,date:w,ds:st(w)})}r++}}return m}if(!l||!l.length){const m={},a=v.filter(A=>{const S=new Date(A);S.setDate(S.getDate()-S.getDay()),S.setHours(0,0,0,0);const R=S.getTime();return m[R]=(m[R]||0)+1,m[R]<=o}),g=[];for(const A of a){if(g.length>=e)break;g.push({n:g.length+1,date:new Date(A),ds:st(A)})}if(g.length>=e)return g;const p=a[a.length-1],r=new Date(p);r.setDate(r.getDate()-r.getDay()),r.setHours(0,0,0,0);const $=r.getTime(),w=a.filter(A=>A.getTime()>=$&&A.getTime()<$+6048e5);let j=1;for(;g.length<e;){for(const A of w){if(g.length>=e)break;const S=new Date(A);S.setDate(S.getDate()+j*7),g.push({n:g.length+1,date:S,ds:st(S)})}j++}return g}const d={},b=[];for(const f of v){if(b.length>=e)break;const m=new Date(f);m.setDate(m.getDate()-m.getDay()),m.setHours(0,0,0,0);const a=m.getTime();d[a]=(d[a]||0)+1,d[a]<=o&&b.push({n:b.length+1,date:new Date(f),ds:st(f)})}if(b.length>=e)return b;const y=v[v.length-1],u=new Date(y);u.setDate(u.getDate()-u.getDay()),u.setHours(0,0,0,0);const x=u.getTime(),z=7*24*60*60*1e3,C={};for(const f of v)if(f.getTime()>=x&&f.getTime()<x+z){const m=f.getDay();C[m]=(C[m]||0)+1}const H={};for(const f of l)H[f]=(H[f]||0)+1;const _=[];for(const[f,m]of Object.entries(H)){const a=m-(C[Number(f)]||0);for(let g=0;g<a;g++)_.push(Number(f))}_.sort((f,m)=>f-m);for(const f of _){if(b.length>=e)break;const m=new Date(u);m.setDate(m.getDate()+f),b.push({n:b.length+1,date:m,ds:st(m)})}let D=1;for(;b.length<e;){for(const f of l){if(b.length>=e)break;const m=new Date(u);m.setDate(m.getDate()+D*7+f),b.push({n:b.length+1,date:m,ds:st(m)})}D++}return b}function Qt(t){return t==="ACDMVOC"?"porwor":"samai"}function Z(t){return String(t??"").split(" ")[0]}function _t(t,i){const s={};for(const e of t){const l=e[i];l&&(s[l]=(s[l]??0)+1)}let c=null,o=0;for(const[e,l]of Object.entries(s))l>o&&(o=l,c=e);return c}function Zt(t){return _t(t,"religion_room")}function te(t){return _t(t,"main_room")}function mt(t){return t>=80?4:t>=75?3.5:t>=70?3:t>=65?2.5:t>=60?2:t>=55?1.5:t>=50?1:0}function kt(t){return t>=3.5?"ดีเยี่ยม":t>=2.5?"ดี":t>=1?"ผ่าน":"ไม่ผ่าน"}async function ee(t){var W,Q;const i=await Rt();Kt(i);const s=parseInt(i.academicYear??i.academic_year??2568),c=parseInt(i.semester??1),{data:o,error:e}=await gt.from("classes").select(`
  ...
  </div>`}const yt=50;function At(t){const{cls:i,ms:s,credit:c,cfg:o,students:e,attMap:l,sessions:M,academicYear:v,semester:d,teacher:b}=t,y=[];for(let u=0;u<e.length;u+=yt){const x=e.slice(u,u+yt);y.push(se(t,x,u+1))}return y.join("")}function se(t,i,s){const{cls:c,ms:o,teacher:e,academicYear:l,semester:M,cfg:v,prefix:d}=t,b=(v==null?void 0:v[`${d}LogoBwUrl`])||(v==null?void 0:v[`${d}LogoUrl`])||(v==null?void 0:v.samaiLogoBwUrl)||(v==null?void 0:v.samaiLogoUrl)||"",y=40,u=i.length+1,x=Math.max(3.5,Math.min(5.5,Math.floor(241/u*10)/10)).toFixed(1),z="3.2mm",C=i.map((D,f)=>{const m=t.attMap[D.id]??{},a=[];for(const[r,$]of Object.entries(m))$!=="present"&&a.push({n:parseInt(r),status:$});a.sort((r,$)=>r.n-$.n);const g=a.length,p=Array.from({length:y},(r,$)=>{const w=a[$];return w?`<td class="${w.status==="absent"?"att-absent":w.status==="leave"?"att-leave":w.status==="sick"?"att-sick":"att-absent"}">${w.n}</td>`:"<td></td>"});return`<tr>
  ...
  </section>`}const it=2;function oe(t,i){const s=Math.ceil(((t==null?void 0:t.length)||0)/it),c=Array.from({length:it},(e,l)=>Array.from({length:s},(M,v)=>{const d=t[l*s+v];return d?{sess:d,week:Math.ceil(d.n/i)}:null})),o=c.map(e=>e.map((l,M)=>{var d,b;if(!l)return null;if(M>0&&((d=e[M-1])==null?void 0:d.week)===l.week)return 0;let v=1;for(let y=M+1;y<s&&((b=e[y])==null?void 0:b.week)===l.week;y++)v++;return v}));return{N:s,colData:c,colRS:o}}const Dt=233;function le(t){const i=Math.max(6,...(t??[]).map(s=>(s.full_name??"").length));return Math.min(50,Math.max(30,i*2.3))}function ie(t,i,s){const o=Math.max(i.length+3,15),e=Math.max(3,Math.min(7,Dt/o)),l=e<3.6?7.5:e<4.4?8.5:e<5.4?9.5:e<6.2?10.5:11.5,M=Array.from({length:o},(d,b)=>{const y=i[b];if(!y)return`<tr>
  ...
  <table class="voc-attendance voc-student-list" style="--att-row-h:${e.toFixed(2)}mm;font-size:${l}px">
    ...
    <tbody>${M.join("")}</tbody>
  </table>`}function ce(t,i){const{N:s,colData:c,colRS:o}=t,e=Math.max(1.6,Math.min(4.55,Dt/Math.max(1,s))),l=e<2.4?6:e<3.2?7:e<4?8:9.5,M=Array.from({length:s},(v,d)=>`<tr>${Array.from({length:it},(y,u)=>{const x=c[u][d],z=o[u][d];if(!x)return'<td class="voc-sched-week"></td><td class="voc-sched-period"></td><td class="voc-sched-date"></td>';const C=z===0?"":`<td class="voc-sched-week voc-center" rowspan="${z}">${x.week}</td>`,H=i==null?void 0:i.has(x.sess.ds);return`${C}<td class="voc-sched-period voc-center">${x.sess.n}</td><td class="voc-sched-date voc-center" style="${H?"color:#c00;font-weight:700;":""}">${$t(x.sess.ds)}</td>`}).join("")}</tr>`);return`
  <table class="voc-attendance voc-schedule-list" style="--att-row-h:${e.toFixed(2)}mm;font-size:${l}px">
    ...
    <tbody>${M.join("")}</tbody>
  </table>`}function re(t){const{students:i,sessions:s}=t,c=s!=null&&s.length?Math.round(s.length/20):1,o=oe(s,c),e=le(i);return`
  <section class="voc-page">
  ...
  </section>`}const ft=31;function de(t,i=8){const s=n(t??"");if(s.length<=i)return s;const c=Math.ceil(s.length/2);let o=s.lastIndexOf(" ",c);return o<=0&&(o=s.indexOf(" ",c)),o<=0?s:`${s.slice(0,o)}<br>${s.slice(o+1)}`}function me(t,i,s){const{cls:c,teacher:o,deptHeadName:e,scoreColumns:l,scoreMap:M,moralScores:v,moralMax:d,moralColName:b}=t,y=a=>a.assignment_type==="คะแนนพิเศษ",u=l.filter(a=>!y(a)),x=l.filter(a=>y(a)),z=u.reduce((a,g)=>a+(g.max_score??0),0),C=z+(d||0),H=!!window._pp5HideScores,_=i.map((a,g)=>{if(H)return`<tr>
      ...
      <table class="voc-eval">
        ...
        <thead>
          ...
          <tr class="voc-h-vertical">
            ${u.map(a=>`<th class="voc-c-obj"><div class="voc-vtext">${de(a.assignment_name??"")}</div></th>`).join("")}
  ...
  </section>`}function pe(t){const{students:i}=t,s=[];for(let c=0;c<i.length;c+=ft)s.push(me(t,i.slice(c,c+ft),c+1));return s.join("")}const ge=46,he=5.3,fe=5.45;function ve(t){const i=Math.max(1,Math.ceil((t.content??"").length/ge));return Math.max(fe,i*he)}function ue(t){const{ms:i,teacher:s,courseDoc:c}=t,o=Array.isArray(c==null?void 0:c.voc_objectives)?c.voc_objectives:[],e=Array.isArray(c==null?void 0:c.voc_schedule)?c.voc_schedule:[],l=o.concat(Array.from({length:Math.max(0,10-o.length)},()=>({objective:"",competency:""}))),M=e.concat(Array.from({length:Math.max(0,20-e.length)},()=>({week:"",content:"",note:""}))),v=297,d=30,b=10,y=9.5,u=10,x=8,z=7,C=7,H=x+l.length*z,_=v-d-b-y-u-H-C,D=v-d-y-C,f=[];let m=[],a=0,g=_;for(const w of M){const j=ve(w);m.length&&a+j>g&&(f.push(m),m=[],a=0,g=D),m.push(w),a+=j}f.push(m);const p=`
...
<body>${t}</body></html>`}function we(t){var D,f,m;(D=document.getElementById("pp5-viewer"))==null||D.remove();const i=((f=t.ms)==null?void 0:f.subject_group)==="ACDMVOC",s=i?[{label:"ดูทั้งหมด",fn:null,all:!0},{label:"หน้าปก",fn:()=>ne(t)},{label:"ไม่มาเรียน/วันที่สอน",fn:()=>re(t)},{label:"คะแนน",fn:()=>pe(t)},{label:"จุดประสงค์/กำหนดการสอน",fn:()=>ue(t)}]:[{label:"ดูทั้งหมด",fn:null,all:!0},{label:"หน้าปก",fn:()=>St(t)},{label:"มาตรฐาน/ตัวชี้วัด",fn:()=>Ct(t)},{label:"บันทึกการไม่มาเรียน",fn:()=>At(t)},{label:"คะแนน",fn:()=>Mt(t)},{label:"วันที่สอน",fn:()=>Ht(t)}],c=s.slice(1).map((a,g)=>g+1),o=!!((m=t.cfg)!=null&&m.pp5PreviewEditEnabled),e={},l=document.createElement("div");l.id="pp5-viewer",l.style.cssText="position:fixed;inset:0;z-index:9999;display:flex;flex-direction:column;background:#374151;";const M=a=>"border:none;border-radius:6px;padding:6px 14px;cursor:pointer;font-family:Sarabun,sans-serif;font-size:13px;font-weight:600;white-space:nowrap;"+(a?"background:#2563eb;color:#fff;":"background:#4b5563;color:#d1d5db;");l.innerHTML=`
    ...
    </div>`,document.body.appendChild(i),i.querySelector("#pp5-cancel").addEventListener("click",()=>i.remove()),i.querySelector("#pp5-open-btn").addEventListener("click",async()=>{const c=i.querySelector('input[name="pp5-cls"]:checked');if(!c){dt("กรุณาเลือกห้อง","warning");return}i.remove(),await ye(parseInt(c.value))}),i.addEventListener("click",c=>{c.target===i&&i.remove()})}export{ye as a,Se as o};
