import{s as G}from"./supabase-BV-W2lsh.js";import{i as J}from"./supabase-errors-BniCCodr.js";const T=1e3,q=14,B=31,Q=3e4,C=document.getElementById("prayer-dashboard-root");let n={days:q,endDate:D(),location:"",roomGroup:"",rows:[],students:[],loading:!1,updatedAt:null};const i=t=>String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");function D(){const t=new Date;return t.setMinutes(t.getMinutes()-t.getTimezoneOffset()),t.toISOString().slice(0,10)}function tt(t){const a=t instanceof Date?new Date(t):new Date(t);return a.setMinutes(a.getMinutes()-a.getTimezoneOffset()),a.toISOString().slice(0,10)}function z(t,a){const[o,s,l]=String(t||D()).split("-").map(Number),r=new Date(o,(s||1)-1,l||1);return r.setDate(r.getDate()+a),tt(r)}function O(t){const a=parseInt(t,10);return Number.isFinite(a)?Math.max(7,Math.min(B,a)):q}function P(t){return{musolla_male:"มูซอลลาชาย",masjid_kuwait:"มัสยิดคูเวต",musolla_female_1:"มูซอลลาหญิง 1",musolla_female_2:"มูซอลลาหญิง 2"}[t]||"ไม่ระบุจุด"}function et(t){return{musolla_male:"bg-blue-50 text-blue-700 border-blue-100",masjid_kuwait:"bg-violet-50 text-violet-700 border-violet-100",musolla_female_1:"bg-pink-50 text-pink-700 border-pink-100",musolla_female_2:"bg-amber-50 text-amber-700 border-amber-100"}[t]||"bg-slate-50 text-slate-600 border-slate-100"}function F(t=""){const a=String(t||"").trim();if(a.startsWith("ปวช."))return"ปวช.";const o=a.match(/^ม\.\d/);return o?o[0]:"ไม่ระบุ"}function at(t){const a=new Map;return t.forEach(o=>{if(!o.student_id)return;const s=a.get(o.student_id);(!s||String(o.created_at||"")>String(s.created_at||""))&&a.set(o.student_id,o)}),Array.from(a.values())}async function U(t,a,o=s=>s){const s=[];for(let l=0;;l+=T){let r=G.from(t).select(a);r=o(r);const{data:c,error:f}=await r.range(l,l+T-1);if(f)throw f;if(s.push(...c??[]),!c||c.length<T)break}return s}async function v(){if(!n.loading){n.loading=!0,_();try{const t=z(n.endDate,-(n.days-1)),a=await st(t,n.endDate);n.students=a.students,n.rows=a.records,n.updatedAt=new Date}catch(t){console.error("Load prayer dashboard failed:",t),C.innerHTML=`
  ...
  `}function lt(t){const a=Math.max(1,...t.map(u=>Math.max(u.pray,u.unique))),o=680,s=190,l=t.length>1?o/(t.length-1):o,r=u=>t.map((m,x)=>{const g=Math.round(x*l),S=Math.round(s-m[u]/a*(s-28)-14);return`${g},${S}`}).join(" "),c=r("pray"),f=r("unique");return`
  ...
  `}function dt(t){const a=Math.max(1,...t.map(o=>o.operatorsCount));return`
    <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
  ...
  `}function it(t,a="total",o="bg-emerald-500",s="ยังไม่มีข้อมูล"){if(!t.length)return`<p class="text-center text-sm text-slate-400 py-8">${i(s)}</p>`;const l=Math.max(1,...t.map(r=>r[a]||0));return`
    <div class="space-y-3">
      ${t.map(r=>`
        <div>
  ...
  `:'<div class="text-center text-sm text-slate-400 py-10">ยังไม่มีข้อมูลแกนนำในช่วงที่เลือก</div>'}function _(){const t=rt(),a=n.updatedAt?n.updatedAt.toLocaleTimeString("th-TH",{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"—",o=t.trendDelta>=0?"emerald":"rose";C.innerHTML=`
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
        ...
        ${it(t.rooms.slice(0,8),"pray","bg-emerald-500","ยังไม่มีข้อมูลห้อง")}
      </div>
      <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
        <h2 class="font-extrabold text-slate-800 text-sm mb-3">จุดละหมาดที่ใช้งาน</h2>
        <div class="space-y-2">
          ${t.locations.length?t.locations.map(s=>`
            <div class="flex items-center justify-between gap-3 rounded-xl border px-3 py-2 ${et(s.key)}">
