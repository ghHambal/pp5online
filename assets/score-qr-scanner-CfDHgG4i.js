import{getMyClasses as Q,getClassStudents as I,getScoreColumns as N,getStudentScores as V,getClassByIdFull as G,getSystemConfig as R,getLifeSkillColumns as F,saveStudentScore as P}from"./api-CWYJTdOa.js";import{a as y,g as $}from"./ui-MMtcTwtt.js";import{i as B}from"./skill-groups-BY1NTbf4.js";import"./supabase-BV-W2lsh.js";import"./impersonation-0xVfgYVY.js";import"./supabase-errors-BniCCodr.js";function O(n,e=[]){if(!n)return!1;const t=n.column_type??"regular";return!(t!=="regular"&&t!=="bonus"||e.includes(n.assignment_name))}async function z(n){var t;const e=[];try{const s=await G(n),i=(t=s==null?void 0:s.master_subjects)==null?void 0:t.subject_group,m=s==null?void 0:s.skill_group;if(["AGM","AGMVOC"].includes(i)&&e.push("คะแนนมาเรียน","คะแนนละหมาด"),B(m)){e.push("การมาเรียน","เดินสวนสนาม","ความสะอาด");try{const a=await R(),b=parseInt(a.academicYear??2568),c=parseInt(a.semester??1);(await F(b,c,"สามัญ")).forEach(o=>{e.includes(o.name)||e.push(o.name)})}catch{}}}catch{}return e}function _(n="success"){try{const e=new(window.AudioContext||window.webkitAudioContext),t=e.createOscillator(),s=e.createGain();t.connect(s),s.connect(e.destination),n==="success"?(t.type="sine",t.frequency.setValueAtTime(880,e.currentTime),s.gain.setValueAtTime(.08,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.12),t.start(),t.stop(e.currentTime+.12)):(t.type="sawtooth",t.frequency.setValueAtTime(150,e.currentTime),s.gain.setValueAtTime(.12,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.3),t.start(),t.stop(e.currentTime+.3))}catch{}}async function D(){return window.Html5Qrcode?window.Html5Qrcode:new Promise((n,e)=>{const t=document.createElement("script");t.src="https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js",t.onload=()=>n(window.Html5Qrcode),t.onerror=()=>e(new Error("โหลดตัวอ่าน QR Code ไม่สำเร็จ")),document.head.appendChild(t)})}function f(n){return String(n??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}async function te(n){let e=[];try{e=await Q(n.id)}catch{e=[]}if(!e.length){y("ยังไม่มีห้องเรียนที่สอนอยู่","warning");return}const t=document.createElement("div");t.className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50",t.innerHTML=`
      ...
      <div class="p-4 overflow-y-auto flex flex-col gap-1.5">
        ${e.map(s=>{var i;return`
          <button data-cid="${s.id}" class="sqp-class-btn text-left px-4 py-3 rounded-xl border border-gray-100 hover:border-indigo-300 hover:bg-indigo-50 transition">
            ...
            <p class="text-xs text-gray-400 truncate">${f(((i=s.master_subjects)==null?void 0:i.subject_name)??"")}</p>
          </button>`}).join("")}
      </div>
    </div>`,document.body.appendChild(t),t.querySelector("#sqp-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",s=>{s.target===t&&t.remove()}),t.querySelectorAll(".sqp-class-btn").forEach(s=>s.addEventListener("click",()=>{const i=e.find(m=>m.id===parseInt(s.dataset.cid));t.remove(),W({classId:i.id,className:i.class_name})}))}async function W(n){const{classId:e,className:t}=n;let s=[],i=[],m=[];try{[s,i,m]=await Promise.all([I(e),N(e),z(e)])}catch(o){y("โหลดข้อมูลห้องไม่สำเร็จ: "+$(o),"error");return}if(!s.length){y("ห้องนี้ยังไม่มีนักเรียน","warning");return}const a=i.filter(o=>O(o,m));if(!a.length){y("ห้องนี้มีแต่คอลัมน์คะแนนอัตโนมัติ (เช่น คะแนนมาเรียน/คะแนนละหมาด/ทักษะชีวิต) ยังไม่มีคอลัมน์ที่ครูสร้างเองให้สแกนบันทึกได้ — เพิ่มคอลัมน์คะแนนใหม่ที่หน้าบันทึกคะแนนก่อนครับ","warning");return}let b=n.initialColumnId&&a.some(o=>o.id===n.initialColumnId)?n.initialColumnId:a[0].id,c=[];try{c=await V(e)}catch{c=[]}const h={};c.forEach(o=>{h[`${o.student_id}:${o.score_column_id}`]=o.score}),Y({classId:e,className:t,students:s,eligibleCols:a,currentColumnId:b,scoreMap:h})}function Y({classId:n,className:e,students:t,eligibleCols:s,currentColumnId:i,scoreMap:m}){var j;(j=document.getElementById("score-scan-overlay"))==null||j.remove();const a=document.createElement("div");a.id="score-scan-overlay",a.className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col",a.innerHTML=`
    <style>
    ...
    <div class="px-4 py-2.5 border-b border-slate-800 flex-shrink-0">
      ...
      <select id="sqs-col-select" class="w-full bg-slate-900 border border-slate-700 text-slate-100 text-sm rounded-xl px-3 py-2">
        ${s.map(r=>`<option value="${r.id}" ${r.id===i?"selected":""}>${f(r.assignment_name)} (เต็ม ${r.max_score??"-"})</option>`).join("")}
      </select>
    </div>`:""}
  ...
  `,document.body.appendChild(a);const b=[];let c=null,h=null,o=0,S=!1;const T=()=>s.find(r=>r.id===i),L=()=>{const r=a.querySelector("#sqs-history-list"),l=a.querySelector("#sqs-history-count");if(l.textContent=`${b.length} คน`,!b.length){r.innerHTML='<p class="text-slate-500 text-center py-1">ยังไม่มีประวัติ</p>';return}r.innerHTML=b.map(p=>`
      ...
      </div>`).join("")},C=a.querySelector("#sqs-col-select");C&&C.addEventListener("change",r=>{i=parseInt(r.target.value)});async function E(){if(c)try{await c.pause(!0)}catch{}}async function H(){if(c)try{c.resume()}catch{}}function M(r){S=!0;const l=T(),p=`${r.id}:${l.id}`,v=m[p],d=a.querySelector("#sqs-score-popup"),w=r.image_url?`<img src="${r.image_url}" class="w-14 h-18 object-cover object-top rounded-xl border border-gray-200" />`:`<div class="w-14 h-18 rounded-xl bg-sky-50 border border-sky-100 text-sky-400 font-bold text-lg flex items-center justify-center">${r.full_name.charAt(0)}</div>`;d.querySelector("#sqs-popup-student").innerHTML=`
      ${w}
      ...
      </div>`,d.querySelector("#sqs-popup-colname").textContent=`${l.assignment_name} (เต็ม ${l.max_score??"-"})${v!=null?" — คะแนนเดิม: "+v:""}`;const u=d.querySelector("#sqs-score-input");u.value=v??"",d.classList.remove("hidden"),setTimeout(()=>u.focus(),50),d.querySelectorAll(".sqs-adj").forEach(x=>{x.onclick=()=>{const k=parseFloat(x.dataset.adj),g=parseFloat(u.value)||0;u.value=Math.round((g+k)*1e3)/1e3}});const q=async()=>{d.classList.add("hidden"),S=!1,await H()};d.querySelector("#sqs-popup-cancel").onclick=q,d.querySelector("#sqs-popup-save").onclick=async()=>{const x=u.value.trim();if(x===""){y("กรุณากรอกคะแนน","warning");return}const k=d.querySelector("#sqs-popup-save");k.disabled=!0;try{const g=await P(n,r.id,l.id,x,{max:l.max_score});m[p]=(g==null?void 0:g.final)??parseFloat(x),b.unshift({name:r.full_name,colName:l.assignment_name,score:m[p]}),L(),y(`บันทึกคะแนน ${r.full_name} แล้ว ✅`,"success"),await q()}catch(g){y("บันทึกไม่สำเร็จ: "+$(g),"error")}finally{k.disabled=!1}}}async function A(r){if(S)return;const l=a.querySelector("#sqs-camera-container"),p=a.querySelector("#sqs-feedback"),v=u=>{l.classList.add(u?"sqs-flash-success":"sqs-flash-error"),setTimeout(()=>l.classList.remove(u?"sqs-flash-success":"sqs-flash-error"),500)};let d=r;if(r.startsWith("SQ:")){const[,u,q]=r.split(":"),x=Math.floor(Date.now()/1e3)-parseInt(q,10);if(x>60||x<-60){_("error"),v(!1),p.innerHTML='<div class="bg-red-950/40 border border-red-800/80 rounded-2xl p-3 text-center text-xs text-red-400">QR Code หมดอายุแล้ว ให้นักเรียนเปิดหน้าใหม่</div>';return}d=u}const w=t.find(u=>u.student_code===d);if(!w){_("error"),v(!1),p.innerHTML='<div class="bg-red-950/40 border border-red-800/80 rounded-2xl p-3 text-center text-xs text-red-400">ไม่พบนักเรียนคนนี้ในห้องนี้</div>';return}_("success"),v(!0),p.innerHTML=`<div class="bg-sky-950/40 border border-sky-800/80 rounded-2xl p-3 text-center text-xs text-sky-300">✓ พบ ${f(w.full_name)} — กรอกคะแนนในป๊อบอัพ</div>`,await E(),M(w)}a.querySelector("#sqs-close").addEventListener("click",async()=>{if(c)try{await c.stop()}catch{}a.remove()}),(async()=>{try{const r=await D();c=new r("sqs-camera-reader"),await c.start({facingMode:"environment"},{fps:25,aspectRatio:1},l=>{l===h&&Date.now()-o<2e3||(h=l,o=Date.now(),A(l))},()=>{})}catch(r){y("ไม่สามารถเปิดกล้องได้: "+$(r),"error"),a.remove()}})()}export{O as isEligibleScoreColumn,W as openScoreScanner,te as openScoreScannerPickClass};
