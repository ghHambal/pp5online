import{getMyClasses as Q,getClassStudents as I,getScoreColumns as N,getStudentScores as V,getClassByIdFull as R,getSystemConfig as F,getLifeSkillColumns as G,saveStudentScore as P}from"./api-CnonnVVn.js";import{a as y,g as $}from"./ui-CHdefT5i.js";import"./supabase-BV-W2lsh.js";import"./impersonation-0xVfgYVY.js";import"./supabase-errors-BniCCodr.js";import"./version.js_v_10.22-mqYkeZoJ.js";function B(n,e=[]){if(!n)return!1;const t=n.column_type??"regular";return!(t!=="regular"&&t!=="bonus"||e.includes(n.assignment_name))}async function O(n){var t;const e=[];try{const s=await R(n),i=(t=s==null?void 0:s.master_subjects)==null?void 0:t.subject_group,m=s==null?void 0:s.skill_group;if(["AGM","AGMVOC"].includes(i)&&e.push("คะแนนมาเรียน","คะแนนละหมาด"),m==="ชีวิต"){e.push("การมาเรียน","เดินสวนสนาม","ความสะอาด");try{const a=await F(),b=parseInt(a.academicYear??2568),c=parseInt(a.semester??1);(await G(b,c,"สามัญ")).forEach(o=>{e.includes(o.name)||e.push(o.name)})}catch{}}}catch{}return e}function _(n="success"){try{const e=new(window.AudioContext||window.webkitAudioContext),t=e.createOscillator(),s=e.createGain();t.connect(s),s.connect(e.destination),n==="success"?(t.type="sine",t.frequency.setValueAtTime(880,e.currentTime),s.gain.setValueAtTime(.08,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.12),t.start(),t.stop(e.currentTime+.12)):(t.type="sawtooth",t.frequency.setValueAtTime(150,e.currentTime),s.gain.setValueAtTime(.12,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.3),t.start(),t.stop(e.currentTime+.3))}catch{}}async function z(){return window.Html5Qrcode?window.Html5Qrcode:new Promise((n,e)=>{const t=document.createElement("script");t.src="https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js",t.onload=()=>n(window.Html5Qrcode),t.onerror=()=>e(new Error("โหลดตัวอ่าน QR Code ไม่สำเร็จ")),document.head.appendChild(t)})}function f(n){return String(n??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}async function ee(n){let e=[];try{e=await Q(n.id)}catch{e=[]}if(!e.length){y("ยังไม่มีห้องเรียนที่สอนอยู่","warning");return}const t=document.createElement("div");t.className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50",t.innerHTML=`
    ...
    </div>`,document.body.appendChild(t),t.querySelector("#sqp-close").addEventListener("click",()=>t.remove()),t.addEventListener("click",s=>{s.target===t&&t.remove()}),t.querySelectorAll(".sqp-class-btn").forEach(s=>s.addEventListener("click",()=>{const i=e.find(m=>m.id===parseInt(s.dataset.cid));t.remove(),D({classId:i.id,className:i.class_name})}))}async function D(n){const{classId:e,className:t}=n;let s=[],i=[],m=[];try{[s,i,m]=await Promise.all([I(e),N(e),O(e)])}catch(o){y("โหลดข้อมูลห้องไม่สำเร็จ: "+$(o),"error");return}if(!s.length){y("ห้องนี้ยังไม่มีนักเรียน","warning");return}const a=i.filter(o=>B(o,m));if(!a.length){y("ห้องนี้มีแต่คอลัมน์คะแนนอัตโนมัติ (เช่น คะแนนมาเรียน/คะแนนละหมาด/ทักษะชีวิต) ยังไม่มีคอลัมน์ที่ครูสร้างเองให้สแกนบันทึกได้ — เพิ่มคอลัมน์คะแนนใหม่ที่หน้าบันทึกคะแนนก่อนครับ","warning");return}let b=n.initialColumnId&&a.some(o=>o.id===n.initialColumnId)?n.initialColumnId:a[0].id,c=[];try{c=await V(e)}catch{c=[]}const h={};c.forEach(o=>{h[`${o.student_id}:${o.score_column_id}`]=o.score}),W({classId:e,className:t,students:s,eligibleCols:a,currentColumnId:b,scoreMap:h})}function W({classId:n,className:e,students:t,eligibleCols:s,currentColumnId:i,scoreMap:m}){var j;(j=document.getElementById("score-scan-overlay"))==null||j.remove();const a=document.createElement("div");a.id="score-scan-overlay",a.className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col",a.innerHTML=`
    <style>
