import{s as u}from"./supabase-BV-W2lsh.js";/* empty css             */import{a as d,g as w}from"./ui-FQqAmrdo.js";import"./version.js_v_10.22-ffVTG8-v.js";const k="pp5_sports_offline_queue_v1",c={events:"sports_events",competitions:"sports_competitions",registrations:"sports_registrations",matches:"sports_matches",totals:"sports_color_totals"};let s={loaded:!1,missingSchema:!1,missingMessage:"",event:null,groups:[],students:[],teachers:[],competitions:[],registrations:[],matches:[],totals:[],search:""};const n=e=>String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),v=e=>{if(!e)return"—";try{return new Date(e+"T00:00:00").toLocaleDateString("th-TH",{day:"numeric",month:"short",year:"numeric"})}catch{return e}};function I(e){document.querySelectorAll("[data-nav]").forEach(t=>{t.classList.toggle("bg-indigo-800",t.dataset.nav===e),t.classList.toggle("text-white",t.dataset.nav===e),t.classList.toggle("text-indigo-200",t.dataset.nav!==e)})}function S(e){const t=document.getElementById("main-content")||document.getElementById("sports-root");t&&(t.innerHTML=e)}function N(e){const t=`${(e==null?void 0:e.code)??""} ${w(e)} ${(e==null?void 0:e.details)??""}`;return/PGRST20|does not exist|Could not find|schema cache|relation .* does not exist/i.test(t)}async function m(e,t){const{data:o,error:r}=await t;if(r){if(N(r))return s.missingSchema=!0,s.missingMessage=`${e}: ${r.message}`,[];throw r}return o??[]}function b(){try{return JSON.parse(localStorage.getItem(k)||"[]")}catch{return[]}}function B(e){localStorage.setItem(k,JSON.stringify(e))}function A(e,t){const o=b();o.push({id:`${Date.now()}_${Math.random().toString(16).slice(2)}`,type:e,payload:t,created_at:new Date().toISOString()}),B(o)}async function H(){const[e,t,o]=await Promise.all([m("house_groups",u.from("house_groups").select("id, name, color_hex, gender, sort_order, teacher_id").order("gender").order("sort_order")),m("students",u.from("students").select("id, student_code, full_name, main_room, religion_room, gender, house_color, sports_shirt_size, image_url, is_active").eq("is_active",!0).order("student_code")),m("teachers",u.from("teachers").select("id, teacher_code, full_name, position, positions").order("full_name"))]);s.groups=e,s.students=t,s.teachers=o}async function T(){s.missingSchema=!1,s.missingMessage="";const e=await m(c.events,u.from(c.events).select("*").order("academic_year",{ascending:!1}).order("created_at",{ascending:!1}).limit(1));if(s.event=e[0]??null,!s.event){s.competitions=[],s.registrations=[],s.matches=[],s.totals=[];return}const t=s.event.id,[o,r,a,p]=await Promise.all([m(c.competitions,u.from(c.competitions).select("*, teachers:responsible_teacher_id(id, teacher_code, full_name)").eq("event_id",t).order("display_order").order("name")),m(c.registrations,u.from(c.registrations).select("*, students(id, student_code, full_name, main_room, gender, house_color, sports_shirt_size, image_url), sports_competitions(id, code, name, category)").eq("event_id",t).order("registered_at",{ascending:!1})),m(c.matches,u.from(c.matches).select("*, sports_competitions(id, code, name, category)").eq("event_id",t).order("scheduled_date",{ascending:!0}).order("scheduled_time",{ascending:!0})),m(c.totals,u.from(c.totals).select("*").eq("event_id",t))]);s.competitions=o,s.registrations=r,s.matches=a,s.totals=p}async function f(){s.loaded=!1,j(),await H(),await T(),s.loaded=!0,j()}function F(e){return s.groups.find(t=>t.name===e)}function D(){const e=new Map;return s.students.forEach(t=>{const o=t.house_color||"__none__";e.has(o)||e.set(o,[]),e.get(o).push(t)}),e}function E(e=""){return['<option value="">— เลือกสี —</option>',...s.groups.map(t=>`<option value="${n(t.name)}" ${t.name===e?"selected":""}>สี${n(t.name)}</option>`)].join("")}function z(e=""){return['<option value="">— ไม่ระบุ —</option>',...s.teachers.map(t=>`<option value="${t.id}" ${String(t.id)===String(e)?"selected":""}>${n(t.full_name)}${t.teacher_code?` (${n(t.teacher_code)})`:""}</option>`)].join("")}function L(e=""){return['<option value="">— เลือกรายการแข่งขัน —</option>',...s.competitions.map(t=>`<option value="${t.id}" ${String(t.id)===String(e)?"selected":""}>${n(t.name)}${t.code?` (${n(t.code)})`:""}</option>`)].join("")}function M(){const e=s.search.trim().toLowerCase();return['<option value="">— เลือกนักเรียน —</option>',...s.students.filter(o=>!e||[o.student_code,o.full_name,o.main_room,o.house_color].some(r=>String(r??"").toLowerCase().includes(e))).slice(0,60).map(o=>`<option value="${o.id}">${n(o.student_code)} · ${n(o.full_name)} · ${n(o.main_room||"—")} · สี${n(o.house_color||"—")}</option>`)].join("")}function G(){return s.missingSchema?`
  ...
  `:""}function J(){const e=D(),t=s.students.filter(p=>p.house_color).length,o=s.matches.filter(p=>p.status==="done").length,r=b().length;return`
  ...
  `}function R(){return!s.event&&!s.missingSchema?`
  ...
  `:""}function U(){var r;const e=D(),t=s.groups.length?s.groups:[...e.keys()].filter(a=>a!=="__none__").map(a=>({name:a,color_hex:"#64748b",gender:""})),o=((r=e.get("__none__"))==null?void 0:r.length)??0;return`
  ...
  `}function Y(){return`
    <div class="grid xl:grid-cols-3 gap-4">
      ...
      <form id="sports-match-form" class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm space-y-3">
        ...
        <div class="grid grid-cols-2 gap-2">
          <select name="team_a_color" required class="border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200">${E()}</select>
          <select name="team_b_color" required class="border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200">${E()}</select>
        </div>
        <div class="grid grid-cols-2 gap-2">
  ...
  `}function Z(){const e=s.competitions;return`
  ...
  `}function K(){const e=s.registrations.slice(0,80);return`
    <section class="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
      <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-3">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ${e.length?e.map(t=>{var a;const o=t.students||{},r=F(o.house_color);return`<tr class="border-t border-gray-50">
                <td class="px-4 py-3">
  ...
  `}function V(){const e=s.matches;return`
    <section class="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
      <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-3">
  ...
  `}function j(){I("sports-admin");const e=document.getElementById("page-title");if(e&&(e.textContent="ระบบกีฬาสี"),!s.loaded&&!s.students.length){S('<div class="min-h-[50vh] flex items-center justify-center text-gray-400">กำลังโหลดระบบกีฬาสี...</div>');return}S(`
  ...
  `),te()}async function O(){const e=new Date().getFullYear()+543,t={name:`กีฬาสีอาซิซเกมส์ ${e}`,academic_year:e,status:"draft"},{error:o}=await u.from(c.events).insert(t);if(o)throw o}function y(e){return Object.fromEntries(new FormData(e).entries())}async function P(e){s.event||await O(),await T();const t={event_id:s.event.id,code:e.code||null,name:e.name,category:e.category||"sport",gender:e.gender||null,max_athletes:e.max_athletes?Number(e.max_athletes):null,venue:e.venue||null,responsible_teacher_id:e.responsible_teacher_id?Number(e.responsible_teacher_id):null,is_active:!0},{error:o}=await u.from(c.competitions).insert(t);if(o)throw o}async function C(e){const t=s.students.find(a=>String(a.id)===String(e.student_id));if(!t)throw new Error("ไม่พบนักเรียน");const o={event_id:s.event.id,competition_id:Number(e.competition_id),student_id:Number(e.student_id),team_color:t.house_color||null,jersey_number:e.jersey_number||null,note:e.note||null},{error:r}=await u.from(c.registrations).insert(o);if(r)throw r}async function Q(e){const t={event_id:s.event.id,competition_id:Number(e.competition_id),team_a_color:e.team_a_color,team_b_color:e.team_b_color,scheduled_date:e.scheduled_date||null,scheduled_time:e.scheduled_time||null,venue:e.venue||null,status:"pending"},{error:o}=await u.from(c.matches).insert(t);if(o)throw o}async function _(e,t,o){try{await o(t),d("บันทึกสำเร็จ","success")}catch(r){if(!navigator.onLine||/fetch|network|Failed to fetch/i.test((r==null?void 0:r.message)??""))A(e,t),d("เน็ตไม่พร้อม บันทึกไว้รอซิงก์แล้ว","warning");else throw d("บันทึกไม่สำเร็จ: "+w(r),"error"),r}}async function X(e){if(e.type==="competition")return P(e.payload);if(e.type==="registration")return C(e.payload);if(e.type==="match")return Q(e.payload)}async function ee(){const e=b();if(!e.length)return;const t=[];for(const o of e)try{await X(o)}catch{t.push(o)}B(t),d(t.length?`ซิงก์สำเร็จบางส่วน ค้าง ${t.length} รายการ`:"ซิงก์ข้อมูลค้างสำเร็จทั้งหมด",t.length?"warning":"success"),await f()}function te(){var t,o,r,a,p,x,h;(t=document.getElementById("sports-refresh"))==null||t.addEventListener("click",()=>f()),(o=document.getElementById("sports-create-event"))==null||o.addEventListener("click",async()=>{try{await O(),d("สร้างกิจกรรมแล้ว","success"),await f()}catch(i){d("สร้างกิจกรรมไม่สำเร็จ: "+w(i),"error")}}),(r=document.getElementById("sports-print-roster"))==null||r.addEventListener("click",()=>window.print()),(a=document.getElementById("sports-sync-queue"))==null||a.addEventListener("click",ee);const e=document.getElementById("sports-student-search");e==null||e.addEventListener("input",()=>{s.search=e.value;const i=document.getElementById("sports-student-select");i&&(i.innerHTML=M())}),(p=document.getElementById("sports-competition-form"))==null||p.addEventListener("submit",async i=>{var $;if(i.preventDefault(),s.missingSchema){d("กรุณารัน patch_sports_module.sql ก่อน","warning");return}const g=i.currentTarget,l=y(g);if(!(($=l.name)!=null&&$.trim())){d("กรุณากรอกชื่อรายการ","warning");return}try{await _("competition",l,P),g.reset(),await f()}catch{}}),(x=document.getElementById("sports-registration-form"))==null||x.addEventListener("submit",async i=>{if(i.preventDefault(),s.missingSchema){d("กรุณารัน patch_sports_module.sql ก่อน","warning");return}if(!s.event){d("กรุณาสร้างกิจกรรมก่อน","warning");return}const g=i.currentTarget,l=y(g);if(!l.competition_id||!l.student_id){d("กรุณาเลือกรายการและนักเรียน","warning");return}try{await _("registration",l,C),g.reset(),s.search="",await f()}catch{}}),(h=document.getElementById("sports-match-form"))==null||h.addEventListener("submit",async i=>{if(i.preventDefault(),s.missingSchema){d("กรุณารัน patch_sports_module.sql ก่อน","warning");return}if(!s.event){d("กรุณาสร้างกิจกรรมก่อน","warning");return}const g=i.currentTarget,l=y(g);if(!l.competition_id||!l.team_a_color||!l.team_b_color){d("กรุณากรอกคู่แข่งขันให้ครบ","warning");return}if(l.team_a_color===l.team_b_color){d("สีคู่แข่งต้องไม่ซ้ำกัน","warning");return}try{await _("match",l,Q),g.reset(),await f()}catch{}})}async function q(){I("sports-admin"),await f()}async function se(){var t;const{data:{session:e}}=await u.auth.getSession();if(!e){window.location.replace("index.html");return}await q(),(t=document.getElementById("sports-page-refresh"))==null||t.addEventListener("click",()=>q())}se();
