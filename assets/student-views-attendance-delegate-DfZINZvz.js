import{J as A,K as T}from"./student-api-BkkkCebX.js";import{getClassStudents as j,getClassAttendanceAll as S,saveAttendanceCell as L}from"./api-CWYJTdOa.js";import{setActiveNav as M,setTitle as I,setContent as n,_htmlEsc as l,ATT_CYCLE as u,ATT_STATUS as N}from"./teacher-views-utils-bZoYj54P.js";import{g as y,a as B}from"./ui-MMtcTwtt.js";import"./supabase-BV-W2lsh.js";import"./supabase-errors-BniCCodr.js";import"./score-display-CQ4dUIPx.js";import"./impersonation-0xVfgYVY.js";import"./skill-groups-BY1NTbf4.js";const $=`<div class="flex justify-center py-16 text-gray-300">
  <svg class="animate-spin h-6 w-6" viewBox="0 0 24 24" fill="none">
    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
  </svg>
</div>`;async function D(c){var d;M("overview"),I("เช็คชื่อแทนครู"),n($);let i;try{i=await A(c)}catch(t){n(`<div class="p-6 text-center text-red-400 text-sm">โหลดข้อมูลไม่สำเร็จ: ${l(y(t))}</div>`);return}const r=i.filter(t=>T(t.period));if(!r.length){n(`
      ...
      </div>`),(d=document.getElementById("ad-back"))==null||d.addEventListener("click",()=>{var t;return(t=window._navTo)==null?void 0:t.call(window,"overview")});return}if(r.length===1){await w(c,r[0]);return}n(`
    <div class="max-w-md mx-auto mt-6 space-y-3 px-4">
      <h2 class="font-bold text-gray-800 text-base">🙋 เลือกห้องที่จะเช็คชื่อ</h2>
      ${r.map((t,p)=>{var a,o;return`
    ...
    <div class="max-w-md mx-auto px-4 py-4 space-y-3 pb-10">
      ...
      <div class="bg-white rounded-2xl border border-gray-100 px-3" id="ad-roster">
        ${o.length?o.map(_).join(""):'<p class="text-center text-gray-400 py-8 text-sm">ยังไม่มีนักเรียนในห้องนี้</p>'}
      </div>
    </div>`),(h=document.getElementById("ad-back2"))==null||h.addEventListener("click",()=>D(c)),document.querySelectorAll(".ad-cell-btn").forEach(e=>{e.addEventListener("click",async()=>{const s=e.closest("[data-sid]"),m=parseInt(s.dataset.sid),C=x[m]??null,b=u[(u.indexOf(C)+1)%u.length];x[m]=b;const g=f(b);e.className=`ad-cell-btn w-11 h-11 rounded-xl border border-gray-200 flex-shrink-0 flex items-center justify-center font-bold text-base ${g.cls} ${g.color}`,e.textContent=g.label;try{await L(r.id,m,t,p,b)}catch(E){B("บันทึกไม่สำเร็จ: "+y(E),"error")}})})}export{D as renderStudentAttendanceDelegate};
