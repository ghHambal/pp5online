import{getActiveLeavePermission as I,getStudentByCode as _,closeLeavePermission as A,getTeacherByCode as q,createLeavePermissionByAnyTeacher as M}from"./api-CnonnVVn.js";import{r as j}from"./leave-monitor.js_v_10.18-BiKjYBa9.js";import{f as N}from"./leave-time-CrS9gT63.js";import{a as g,g as $}from"./ui-CHdefT5i.js";import{setActiveNav as R,setTitle as H,setContent as D,_htmlEsc as m}from"./teacher-views-utils-B68DuafG.js";import"./supabase-BV-W2lsh.js";import"./impersonation-0xVfgYVY.js";import"./supabase-errors-BniCCodr.js";import"./version.js_v_10.22-mqYkeZoJ.js";let u=null,b=null,E=!1;const k=`
...
`;function B(){try{const e=new(window.AudioContext||window.webkitAudioContext),n=e.createOscillator(),s=e.createGain();n.connect(s),s.connect(e.destination),n.type="sine",n.frequency.setValueAtTime(880,e.currentTime),s.gain.setValueAtTime(.1,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.15),n.start(e.currentTime),n.stop(e.currentTime+.15)}catch(e){console.warn("Play audio failed:",e)}}function T(){try{const e=new(window.AudioContext||window.webkitAudioContext),n=e.createOscillator(),s=e.createGain();n.connect(s),s.connect(e.destination),n.type="sawtooth",n.frequency.setValueAtTime(150,e.currentTime),s.gain.setValueAtTime(.15,e.currentTime),s.gain.exponentialRampToValueAtTime(.01,e.currentTime+.4),n.start(e.currentTime),n.stop(e.currentTime+.4)}catch(e){console.warn("Play audio failed:",e)}}function P(){try{u!=null&&u.pause&&u.pause(!0)}catch(e){console.warn("Pause scanner error:",e)}}function V(){try{u!=null&&u.resume&&u.resume()}catch(e){console.warn("Resume scanner error:",e)}}function h({resume:e=!0}={}){const n=document.getElementById("leave-scan-modal");n&&n.remove(),b&&(clearInterval(b),b=null),E=!1,e&&V()}function w(e,{tone:n="indigo",maxWidth:s="max-w-md"}={}){var a;(a=document.getElementById("leave-scan-modal"))==null||a.remove();const i={indigo:"border-indigo-100",emerald:"border-emerald-100",red:"border-red-100",amber:"border-amber-100"}[n]||"border-gray-100",o=document.createElement("div");return o.id="leave-scan-modal",o.className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/65 p-4 animate-fade",o.innerHTML=`
    <div class="w-full ${s} max-h-[92vh] overflow-y-auto rounded-3xl bg-white border ${i} shadow-2xl">
      ${e}
    </div>
  `,document.body.appendChild(o),o}function O(e){w(`
  ...
  `)}async function F(e){var i,o;let n=null;try{n=await _(e)}catch(a){console.warn("getStudentByCode error:",a)}const s=w(`
  ...
  `,{tone:"red"});(i=s.querySelector("#btn-leave-scan-next"))==null||i.addEventListener("click",()=>h()),(o=s.querySelector("#btn-leave-scan-issue-new"))==null||o.addEventListener("click",()=>{h({resume:!1}),Q(n)})}function Q(e){const n=["🚽 ไปห้องน้ำ","💊 ไปห้องพยาบาล","🏢 ไปฝ่ายปกครอง/ธุรการ","✏️ อื่นๆ"],s=[5,10,15,30];let i=n[0],o=10;const a=w(`
        ...
        <div class="grid grid-cols-5 gap-1.5">
          ${s.map(r=>`
            <button class="btn-issue-duration text-[11px] font-semibold py-2 border rounded-xl transition text-center
              ${r===10?"bg-indigo-600 text-white border-indigo-600":"bg-white text-gray-600 border-gray-200 hover:border-gray-400"}"
              data-duration="${r}">${r} น.
            </button>
          `).join("")}
  ...
  `,{tone:"indigo",maxWidth:"max-w-sm"}),p=a.querySelector("#input-issue-custom-reason");a.querySelectorAll(".btn-issue-reason").forEach(r=>{r.addEventListener("click",()=>{a.querySelectorAll(".btn-issue-reason").forEach(x=>{x.className="btn-issue-reason text-xs font-semibold px-3 py-2 border rounded-xl bg-white text-gray-600 border-gray-200 hover:border-gray-400 text-center"}),r.className="btn-issue-reason text-xs font-semibold px-3 py-2 border rounded-xl bg-indigo-600 text-white border-indigo-600 text-center",i=r.dataset.reason,i==="✏️ อื่นๆ"?(p.classList.remove("hidden"),p.focus()):p.classList.add("hidden")})});const y=a.querySelector("#div-issue-custom-duration"),f=a.querySelector("#input-issue-custom-duration");a.querySelectorAll(".btn-issue-duration").forEach(r=>{r.addEventListener("click",()=>{a.querySelectorAll(".btn-issue-duration").forEach(v=>{v.className="btn-issue-duration text-[11px] font-semibold py-2 border rounded-xl bg-white text-gray-600 border-gray-200 hover:border-gray-400 text-center"}),r.className="btn-issue-duration text-[11px] font-semibold py-2 border rounded-xl bg-indigo-600 text-white border-indigo-600 text-center";const x=r.dataset.duration;x==="custom"?(y.classList.remove("hidden"),f.focus()):(y.classList.add("hidden"),o=parseInt(x))})}),a.querySelector("#btn-issue-leave-close").addEventListener("click",()=>h());const l=a.querySelector("#btn-issue-leave-submit");l.addEventListener("click",async()=>{if(l.disabled)return;let r=i;if(i==="✏️ อื่นๆ"&&(r=p.value.trim(),!r)){g("กรุณาระบุเหตุผลในการขออนุญาต","warning");return}let x=o;const v=a.querySelector(".btn-issue-duration.bg-indigo-600");if(v&&v.dataset.duration==="custom"){const t=parseInt(f.value.trim());if(isNaN(t)||t<=0){g("กรุณาระบุระยะเวลากรอกเป็นจำนวนนาทีที่ถูกต้อง (มากกว่า 0)","warning");return}x=t}const d=a.querySelector("#input-issuer-teacher-code").value.trim();if(!d){g("กรุณากรอกรหัสครูผู้ออกใบอนุญาต","warning");return}try{l.disabled=!0,l.textContent="กำลังตรวจสอบรหัสครู...",l.classList.add("opacity-70","cursor-not-allowed");const t=await q(d);if(!t){g("ไม่พบรหัสครูนี้ในระบบ กรุณาตรวจสอบอีกครั้ง","error"),l.disabled=!1,l.textContent="🚪 อนุมัติให้ออกนอกห้อง",l.classList.remove("opacity-70","cursor-not-allowed");return}l.textContent="กำลังบันทึก...",await M(e.id,t.id,r,x),B(),g(`ออกใบอนุญาตให้ ${e.full_name} เรียบร้อย โดย ${t.full_name}`,"success"),S(e.student_code)}catch(t){T(),g("บันทึกไม่สำเร็จ: "+$(t),"error"),l.disabled=!1,l.textContent="🚪 อนุมัติให้ออกนอกห้อง",l.classList.remove("opacity-70","cursor-not-allowed")}})}function z(e){var s;(s=w(`
    <div class="p-6 text-center space-y-5">
  ...
  `,{tone:"red"}).querySelector("#btn-leave-scan-next"))==null||s.addEventListener("click",()=>h())}function W(e){var s,i;(i=w(`
    <div class="p-6 text-center space-y-5">
    ...
    <div class="p-5 sm:p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 gap-3 bg-gray-50 rounded-2xl p-4 text-xs">
        <div class="space-y-0.5">
          ...
          <span class="font-bold text-gray-700 block truncate">${m(((r=e.teachers)==null?void 0:r.full_name)||"ไม่ระบุ")}</span>
        </div>
        <div class="space-y-0.5">
  ...
  `,{tone:o.tone}),p=()=>{const d=i(),t=a.querySelector("#leave-scan-status-badge"),c=a.querySelector("#leave-scan-timer-label"),L=a.querySelector("#leave-scan-timer-text");t&&(t.className=`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${d.statusColorCls}`,t.textContent=d.statusTitle),c&&(c.textContent=d.countdown.label),L&&(L.className=`text-3xl font-black font-mono ${d.timerCls}`,L.textContent=d.countdown.timerText),d.countdown.isBeyondLimit&&b&&(clearInterval(b),b=null)};(x=a.querySelector("#btn-leave-scan-next"))==null||x.addEventListener("click",()=>h()),(v=a.querySelector("#btn-inspector-return"))==null||v.addEventListener("click",async d=>{const t=d.currentTarget;if(!t.disabled){t.disabled=!0,t.textContent="กำลังบันทึก...",t.classList.add("opacity-70","cursor-not-allowed");try{b&&(clearInterval(b),b=null),await A(e.id,"returned"),g("บันทึกการส่งกลับเข้าห้องเรียบร้อย","success"),W(e)}catch(c){t.disabled=!1,t.textContent="✅ บันทึกกลับเข้าห้อง",t.classList.remove("opacity-70","cursor-not-allowed"),g("บันทึกไม่สำเร็จ: "+$(c),"error")}}}),p(),b=setInterval(p,1e3)}async function re(e){R("student-leave-scanner"),H("ตรวจสอบใบอนุญาตออกนอกห้อง"),C(),D(`
  ...
  `);const n=document.getElementById("input-search-student-code"),s=document.getElementById("btn-search-leave-code"),i=document.getElementById("btn-toggle-scanner"),o=document.getElementById("scanner-placeholder-text"),a=document.getElementById("leave-view-scan-tab"),p=document.getElementById("leave-view-monitor-tab"),y=document.getElementById("leave-scanner-panel"),f=document.getElementById("leave-monitor-panel"),l=document.getElementById("leave-monitor-widget");s.addEventListener("click",()=>{const t=n.value.trim();if(!t){g("กรุณากรอกรหัสนักเรียน","warning");return}S(t)}),n.addEventListener("keydown",t=>{if(t.key==="Enter"){const c=n.value.trim();c&&S(c)}});let r=!1;const x=async t=>{const c=t==="monitor";a.className=c?"px-4 py-2 rounded-xl text-gray-500 hover:text-gray-700 text-xs font-bold transition":"px-4 py-2 rounded-xl bg-white text-indigo-700 shadow-sm text-xs font-bold transition",p.className=c?"px-4 py-2 rounded-xl bg-white text-indigo-700 shadow-sm text-xs font-bold transition":"px-4 py-2 rounded-xl text-gray-500 hover:text-gray-700 text-xs font-bold transition",y.classList.toggle("hidden",c),f.classList.toggle("hidden",!c),c&&(r&&d(),await j(l,{title:"🚪 ติดตามใบอนุญาตออกนอกห้อง",subtitle:"รายการที่ฉันเป็นผู้อนุญาต พร้อมแดชบอร์ดแนวโน้ม",teacherId:e.id,analyticsDays:14,externalUrl:"public-monitor.html"}))};a.addEventListener("click",()=>x("scan")),p.addEventListener("click",()=>x("monitor")),i.addEventListener("click",async()=>{r?d():await v()});async function v(){try{o.textContent="กำลังเตรียมกล้อง...";const t=await U();u=new t("leave-camera-reader");const c=document.getElementById("scanner-viewfinder");await u.start({facingMode:"environment"},{fps:25,aspectRatio:1},L=>{S(L)},()=>{}),r=!0,i.innerHTML=`${k}<span>ปิดกล้อง</span>`,i.className="px-3.5 py-2 bg-red-500 hover:bg-red-600 active:scale-[0.98] text-white rounded-2xl font-bold text-xs shadow-md transition-all flex items-center gap-1.5",o.classList.add("hidden"),c&&c.classList.remove("hidden")}catch(t){console.error(t),g("ไม่สามารถเปิดใช้งานกล้องได้: "+$(t),"error"),o.textContent="ไม่สามารถเปิดกล้องได้: "+$(t)}}function d(){u&&u.stop().then(()=>{r=!1,i.innerHTML=`${k}<span>เปิดกล้อง</span>`,i.className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white rounded-2xl font-bold text-xs shadow-md transition-all flex items-center gap-1.5",o.textContent="กล้องถูกปิดใช้งานแล้ว",o.classList.remove("hidden");const t=document.getElementById("scanner-viewfinder");t&&t.classList.add("hidden"),u=null}).catch(t=>{console.warn("Stop scanner error:",t)})}}async function U(){return window.Html5Qrcode?window.Html5Qrcode:new Promise((e,n)=>{const s=document.createElement("script");s.src="https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js",s.onload=()=>e(window.Html5Qrcode),s.onerror=()=>n(new Error("ดาวน์โหลดตัวอ่านสแกนเนอร์ไม่สำเร็จ")),document.head.appendChild(s)})}async function S(e){const n=document.getElementById("leave-scan-result");if(n&&!E){E=!0,P(),n.classList.add("hidden"),n.innerHTML="",O(e);try{const s=await I(e);if(!s){T(),await F(e);return}B(),G(s)}catch(s){console.error(s),T(),z(s.message||"เกิดข้อผิดพลาดในการดึงข้อมูลใบอนุญาต")}}}function C(){var e;(e=document.getElementById("leave-scan-modal"))==null||e.remove(),E=!1,b&&(clearInterval(b),b=null),u&&(u.stop().catch(()=>{}),u=null)}window.addEventListener("hashchange",C);window._cleanupLeaveScanner=C;export{C as cleanupLeaveScanner,re as renderStudentLeaveScanner};
