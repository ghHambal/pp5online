import{getClassStudents as H}from"./api-CnonnVVn.js";import{q as O,c as Q}from"./quiz-api-BIDUVPR5.js";import{_htmlEsc as x}from"./teacher-views-utils-Ceiaeijy.js";import"./supabase-BV-W2lsh.js";import"./impersonation-0xVfgYVY.js";import"./supabase-errors-BniCCodr.js";import"./ui-DI1UEpN2.js";import"./version.js_v_10.22-DxLTis1W.js";const f=[{min:80,label:"80-100%",color:"#10b981"},{min:60,label:"60-79%",color:"#34d399"},{min:40,label:"40-59%",color:"#fbbf24"},{min:20,label:"20-39%",color:"#fb923c"},{min:0,label:"0-19%",color:"#ef4444"}];async function V(c){var k;(k=document.getElementById("quiz-analytics-modal"))==null||k.remove();const o=document.createElement("div");o.id="quiz-analytics-modal",o.className="fixed inset-0 z-[95] flex flex-col bg-gray-50",o.innerHTML=`
    <div class="flex items-center gap-3 px-5 py-4 bg-white border-b border-gray-100 shadow-sm flex-shrink-0">
      ...
      <div class="flex-1 min-w-0">
        <h2 class="font-bold text-gray-800 truncate">📊 สถิติ: ${x(c.title)}</h2>
      </div>
  ...
  `,document.body.appendChild(o),o.querySelector("#qa-close").addEventListener("click",()=>o.remove());const[{attempts:l,violations:j},b,S]=await Promise.all([O(c.id),Q(c.bank_id),H(c.class_id).catch(()=>[])]),m=Object.fromEntries(S.map(t=>[t.id,t]));Object.fromEntries(b.map(t=>[t.id,t]));const p=l.map(t=>t.score_pct).filter(t=>t!=null),h=p.length?p.reduce((t,e)=>t+e,0)/p.length:null,y=f.map(t=>({...t,count:p.filter(e=>{var r;return e>=t.min&&(((r=f[f.indexOf(t)-1])==null?void 0:r.min)??101)>e}).length})),C=Math.max(1,...y.map(t=>t.count)),u={};l.forEach(t=>{t.score_pct!=null&&(u[t.student_id]==null||t.score_pct>u[t.student_id])&&(u[t.student_id]=t.score_pct)});const q=Object.entries(u).map(([t,e])=>({student:m[t],pct:e})).sort((t,e)=>e.pct-t.pct);let v=null,$=0;const w=q.map((t,e)=>(t.pct!==v&&($=e+1,v=t.pct),{...t,rank:$})),A={1:"🥇",2:"🥈",3:"🥉"},B=l.map(t=>Array.isArray(t.question_order)?new Set(t.question_order):new Set),I=b.map(t=>{let e=0,r=0;const a=new Array(t.choices.length).fill(0);l.forEach((M,L)=>{var E;if(!B[L].has(t.id))return;const i=(E=M.answers)==null?void 0:E[t.id];i!=null&&(e++,i===t.correct_choice_index&&r++,i>=0&&i<a.length&&a[i]++)});const s=e>0?Math.round(r/e*100):null,n=s==null?null:s<30?"hard":s>90?"easy":null;return{question:t,attempted:e,correct:r,pctCorrect:s,choiceCounts:a,flag:n}}),d={};j.forEach(t=>{var e;(d[e=t.attempt_id]??(d[e]=[])).push(t)});const g=l.filter(t=>(d[t.id]??[]).length>0).map(t=>({student:m[t.student_id],count:d[t.id].length,events:d[t.id]})),_=document.getElementById("qa-body");_&&(_.innerHTML=`
    <div class="max-w-3xl mx-auto space-y-5">
      <div class="grid grid-cols-3 gap-3">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 text-center">
          <p class="text-2xl font-extrabold text-gray-800">${l.length}</p>
          ...
          ${I.map((t,e)=>`
            <div class="border border-gray-100 rounded-xl p-3">
              ...
              ${t.attempted>0?`<div class="space-y-1">
                ${t.question.choices.map((r,a)=>{const s=Math.round((t.choiceCounts[a]??0)/t.attempted*100),n=a===t.question.correct_choice_index;return`<div class="flex items-center gap-2">
                    <span class="text-xs w-4 flex-shrink-0">${n?"✓":"○"}</span>
                    <span class="text-xs text-gray-600 flex-1 truncate">${x(r)}</span>
  ...
  `,document.getElementById("qa-export").addEventListener("click",()=>{const t=[["รหัสนักเรียน","ชื่อ-สกุล","สถานะ","คะแนน(%)","จำนวนครั้งออกนอกหน้าสอบ"]];l.forEach(s=>{const n=m[s.student_id];t.push([(n==null?void 0:n.student_code)??"",(n==null?void 0:n.full_name)??"",s.status,s.score_pct??"",s.violation_count??0])});const e=t.map(s=>s.map(n=>`"${String(n).replace(/"/g,'""')}"`).join(",")).join(`
`),r=encodeURI("data:text/csv;charset=utf-8,"+e),a=document.createElement("a");a.href=r,a.download=`quiz_${c.title}_analytics.csv`,document.body.appendChild(a),a.click(),document.body.removeChild(a)}))}export{V as openQuizAnalytics};
