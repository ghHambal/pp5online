...
export async function getClassSessionDOWs(classId) {
  // นับ "จำนวนคาบที่เช็คชื่อได้ต่อสัปดาห์" — คูณด้วย span_periods ตั้งใจ เพราะครูที่มีคาบ
  // ต่อเนื่องกันจริง (เช่น 2 คาบติดกัน) ต้องเช็คชื่อแยกทีละคาบ ไม่ใช่เช็คครั้งเดียวจบทั้งบล็อก
  const { data, error } = await supabase
    .from('class_schedule_links')
