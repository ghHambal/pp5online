...
function renderTeamAthletesTab(body,data,card){
  ...
  let query='',sportId=''
  const draw=()=>{const q=query.trim().toLowerCase();const filtered=regs.filter(r=>{const rid=String(r.sport_id||r.sports?.id||'');if(sportId&&rid!==sportId)return false;return !q||[r.students?.full_name,r.students?.student_code,r.students?.main_room,r.sports?.name,r.sports?.code,r.jersey_number].some(v=>String(v||'').toLowerCase().includes(q))});body.querySelector('#athlete-result-count').textContent=`แสดง ${filtered.length} จาก ${regs.length} รายการ`;body.querySelector('#athlete-rows').innerHTML=filtered.map(r=>`<tr class="border-b line"><td class="p-2"><b>${esc(r.students?.full_name||'—')}</b><p class="text-xs muted">${esc(r.students?.student_code||'')} · ${esc(r.students?.main_room||'')}</p></td><td class="p-2">${esc(r.sports?.name||'—')}</td><td class="p-2 text-center font-bold">${esc(r.jersey_number||'—')}</td><td class="p-2"><div class="flex justify-end gap-1"><button data-athlete-qr="${r.id}" class="px-2 py-1.5 rounded-lg team-sub text-xs font-bold">🔳 QR</button>${canEdit?`<button data-athlete-edit="${r.id}" class="px-2 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold">✏️ แก้ไข</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="4" class="p-8 text-center muted">ไม่พบข้อมูลที่ตรงกับเงื่อนไข</td></tr>';body.querySelectorAll('[data-athlete-qr]').forEach(b=>b.onclick=()=>openTeamAthleteQR(regs.find(r=>String(r.id)===b.dataset.athleteQr),c));body.querySelectorAll('[data-athlete-edit]').forEach(b=>b.onclick=()=>openTeamAthleteEdit(regs.find(r=>String(r.id)===b.dataset.athleteEdit),draw))}
