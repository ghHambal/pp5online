...
// ไหนก็ได้ อีกฝั่งเห็นอัตโนมัติ — ค่าเริ่มต้นตรงกับของเดิมทั้งสองระบบ
const DEFAULT_SHIRT_SIZES = [
  { code: 'S', chest: 36 }, { code: 'M', chest: 38 }, { code: 'L', chest: 40 },
  { code: 'XL', chest: 42 }, { code: 'XXL', chest: 44 }, { code: '3XL', chest: 46 },
]
  ...
  try { const {event,cfg,shirtSizes}=await context();if(cfg?.shirt_summary_enabled===false){el.innerHTML='<div class="text-center py-16">แอดมินปิดหน้าสรุปยอดไว้</div>';return}
    ...
    if(isAdmin){
      ...
      const drawSizeRows=()=>{
        ...
        box.innerHTML=sizeRows.map((sz,i)=>`<div class="flex items-center gap-2" data-size-row="${i}"><input data-size-code value="${esc(sz.code)}" placeholder="รหัสไซซ์ เช่น M" class="w-24 border rounded-lg px-2 py-1.5 text-xs"><input data-size-chest type="number" min="0" value="${esc(sz.chest)}" placeholder="รอบอก" class="w-24 border rounded-lg px-2 py-1.5 text-xs"><span class="text-xs text-gray-400">นิ้ว</span><button type="button" data-size-remove class="ml-auto w-8 h-8 rounded-lg border text-red-600 flex items-center justify-center flex-shrink-0">✕</button></div>`).join('')
