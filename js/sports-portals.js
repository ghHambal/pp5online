...
// ไหนก็ได้ อีกฝั่งเห็นอัตโนมัติ — ค่าเริ่มต้นตรงกับตารางไซซ์จริงจากร้านตัดเสื้อ (รอบอก+ความยาวตัว
// หน่วยนิ้ว) ใช้เลขชุดเดียวกันทั้งชาย/หญิง ต่างกันแค่ทรงตัด (ชายแขนสั้น, หญิงทรงยาว)
const DEFAULT_SHIRT_SIZES = [
  { code: 'SS', chest: 34, length: 26 }, { code: 'S', chest: 36, length: 26 },
  { code: 'M', chest: 38, length: 27 }, { code: 'L', chest: 40, length: 28 },
  { code: 'XL', chest: 42, length: 29 }, { code: '2X', chest: 44, length: 30 },
  { code: '3X', chest: 46, length: 31 }, { code: '4X', chest: 48, length: 32 },
  { code: '5X', chest: 50, length: 32 },
]
...
export async function renderStudentSportsHome(student, tab='overview') {
  ...
  try {
    ...
    const sizes=shirtSizes.map(s=>s.code)
...
async function _renderAdvisorSizeTab(body,teacher,rooms,roomNames) {
  body.innerHTML='<div class="py-12 text-center text-gray-400">กำลังโหลด...</div>'
  try {
    const {event,shirtSizes}=await context()
  ...
  try { const {event,cfg,shirtSizes}=await context();if(cfg?.shirt_summary_enabled===false){el.innerHTML='<div class="text-center py-16">แอดมินปิดหน้าสรุปยอดไว้</div>';return}
    ...
    if(isAdmin){
      ...
      const drawSizeRows=()=>{
        ...
        box.innerHTML=sizeRows.map((sz,i)=>`<div class="flex items-center gap-2" data-size-row="${i}"><input data-size-code value="${esc(sz.code)}" placeholder="รหัสไซซ์ เช่น M" class="w-20 border rounded-lg px-2 py-1.5 text-xs"><input data-size-chest type="number" min="0" value="${esc(sz.chest)}" placeholder="รอบอก" class="w-20 border rounded-lg px-2 py-1.5 text-xs"><span class="text-xs text-gray-400">นิ้ว(รอบอก)</span><input data-size-length type="number" min="0" value="${esc(sz.length ?? '')}" placeholder="ยาวตัว" class="w-20 border rounded-lg px-2 py-1.5 text-xs"><span class="text-xs text-gray-400 flex-1">นิ้ว(ยาวตัว)</span><button type="button" data-size-remove class="w-8 h-8 rounded-lg border text-red-600 flex items-center justify-center flex-shrink-0">✕</button></div>`).join('')
        box.querySelectorAll('[data-size-row]').forEach(rowEl=>{
          ...
          rowEl.querySelector('[data-size-length]').addEventListener('input',e=>{sizeRows[i].length=e.target.value})
          rowEl.querySelector('[data-size-remove]').addEventListener('click',()=>{sizeRows.splice(i,1);drawSizeRows()})
        })
      }
      drawSizeRows()
      el.querySelector('#shirt-size-add').addEventListener('click',()=>{sizeRows.push({code:'',chest:'',length:''});drawSizeRows()})
    }
