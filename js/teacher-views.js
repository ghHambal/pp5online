...
export async function renderTeacherOverview(teacher, homeroomRooms = []) {
  ...
  const [subjects, classes, cfg, pendingRequests, donationRequests, svNotifs, todayDuty, dutyGrade, shirtBtnState] = await Promise.all([
    ...
    teacher ? import('./sports-portals.js?v=10.22.654').then(m => m.getTeacherShirtButtonState(teacher)).catch(() => ({ visible: false, enabled: false })) : Promise.resolve({ visible: false, enabled: false }),
  ])
  ...
  window._openTeacherShirtModal = async () => {
    const { openTeacherShirtSizeModal } = await import('./sports-portals.js?v=10.22.654')
  ...
  const sidebarTileColors = {
    ...
    'sports-fund': ['#C8ECC9', '#67B96A'], 'sports-overview': ['#C6E6FA', '#4F9BD6'], 'sports-evaluation': ['#FBE1C6', '#D68A3F'], 'shirt-vote': ['#E2D3F5', '#9663D1'],
