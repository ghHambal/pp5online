...
export async function renderTeacherOverview(teacher, homeroomRooms = []) {
  ...
  const [subjects, classes, cfg, pendingRequests, donationRequests, svNotifs, todayDuty, dutyGrade, shirtBtnState] = await Promise.all([
    ...
    teacher ? import('./sports-portals.js?v=10.22.642').then(m => m.getTeacherShirtButtonState(teacher)).catch(() => ({ visible: false, enabled: false })) : Promise.resolve({ visible: false, enabled: false }),
  ])
  ...
  window._openTeacherShirtModal = async () => {
    const { openTeacherShirtSizeModal } = await import('./sports-portals.js?v=10.22.642')
