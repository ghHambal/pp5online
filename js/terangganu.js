...
อ่านข้อมูลจากรูปที่แนบมาแล้วตอบเป็น JSON เท่านั้น (ไม่มีข้อความอื่นเลย) ตามรูปแบบนี้:
{
  ...
  "national_id": "เลขประจำตัวประชาชน 13 หลัก อ่านได้เฉพาะจากบัตรประชาชนไทยเท่านั้น (หรือ null ถ้ารูปเป็นพาสปอร์ตหรืออ่านไม่ชัด)",
  "passport_number": "เลขที่หนังสือเดินทาง อ่านได้เฉพาะจากพาสปอร์ตเท่านั้น (หรือ null ถ้ารูปเป็นบัตรประชาชนหรือไม่มี)",
...
function renderParticipants(){
  ...
  document.getElementById('print-student-roster-internal').addEventListener('click',()=>openRosterPrint('student',rows,'internal'))
  document.getElementById('print-student-roster-official').addEventListener('click',()=>openRosterPrint('student',rows,'official'))
}
...
// เอกสารส่งหน่วยงานราชการ — ตัดชื่อหมู่/กลุ่มด้านหลังห้องออก เหลือแค่ "ม.X/Y" (เช่น "ม.1/10 Usaha" → "ม.1/10")
function officialRoomLabel(mainRoom){
  const text=String(mainRoom||'').trim()
  const match=text.match(/(?:ม\.?\s*)?([1-6])\s*[\/.]\s*([0-9]+)/i)
  return match?`ม.${match[1]}/${match[2]}`:text
}
function openRosterPrint(type,rows,mode='internal'){
  const isStudent=type==='student',isOfficial=mode==='official',e=ctx.event||{},logo=e.receipt_logo_url
  ...
  // ส่งราชการ: ตัดคอลัมน์ ไซซ์เสื้อ + ค่าค่าย ออกไปเลย (ข้อมูลภายในที่หน่วยงานภายนอกไม่จำเป็นต้องเห็น)
  let body,head,colgroup
  if(!isStudent){
    body=rows.map(({t,r},i)=>`<tr><td>${i+1}</td><td>${esc(t?.teacher_code||'')}</td><td class="name">${esc(t?.full_name)}</td><td>${esc(t?.position||t?.dept||t?.subject_group||'')}</td><td>${esc(r?.phone||t?.phone||'')}</td><td>${esc(r?.shirt_size||'')}</td><td class="check">${r?'✓':''}</td><td></td></tr>`).join('')
  ...
  // ใช้ภายใน: ตัดชื่อยาวด้วย "…" ให้ทุกแถวสูงเท่ากัน สะดวกไล่ตรวจความครบถ้วน
  // ส่งราชการ: ห้ามตัดข้อมูล ปล่อยให้ชื่อตกบรรทัดได้ตามจริง + ฟอนต์ราชการ + ช่องอ้างอิงหนังสือ + ช่องลงนาม
  const nameOverflowCss=isOfficial?'':'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'
  const modeLabel=isOfficial?'(สำหรับส่งหน่วยงานราชการ)':'(สำหรับใช้ภายใน — ตรวจสอบความครบถ้วน)'
  const fontFamily=isOfficial?`'TH Sarabun New','TH SarabunPSK',Arial,sans-serif`:`Arial,'Sarabun',sans-serif`
  const tableFontSize=isOfficial?'12px':'10.5px'
  const thFontSize=isOfficial?'11px':'10px'
  const refBlock=isOfficial?`<div class="ref"><p>แนบท้ายหนังสือที่ .......................................... ลงวันที่ ..........................................</p><p>เรื่อง ..............................................................................................................</p></div>`:''
  const signBlock=isOfficial?`<div class="sign"><div class="sign-col"><p>ลงชื่อ......................................................</p><p>(......................................................)</p><p>ครูผู้ควบคุมคณะ</p></div><div class="sign-col"><p>ลงชื่อ......................................................</p><p>(......................................................)</p><p>ผู้อำนวยการโรงเรียน</p></div></div><p class="sign-date">วันที่ .................... เดือน .......................................... พ.ศ. ....................</p>`:''
  const html=`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>ใบรายชื่อ${isStudent?'นักเรียน':'ครู'} TERANGGANU 2026${isOfficial?' (ส่งราชการ)':''}</title><style>@page{size:A4 landscape;margin:8mm}*{box-sizing:border-box}body{font-family:${fontFamily};color:#111;margin:0;font-size:14px}.head{text-align:center}.logo{width:88px;height:88px;object-fit:contain;filter:grayscale(1);margin:auto}.head h1{font-size:23px;line-height:1.3;margin:5px}.head p{margin:4px;font-size:16px;font-weight:600}.head .mode{font-size:12px;font-weight:400;color:#555}.ref{margin:14px 0 2px;font-size:14px}.ref p{margin:2px 0}table{width:100%;border-collapse:collapse;margin-top:14px;font-size:${tableFontSize};line-height:1.25;table-layout:fixed}thead{display:table-header-group}tr{break-inside:avoid;page-break-inside:avoid}th,td{border:1.2px solid #111;padding:5px 4px;text-align:center;height:26px;vertical-align:middle;overflow-wrap:break-word}th{background:#e8e8e8;font-size:${thFontSize};font-weight:700}.name{text-align:left;font-weight:600;${nameOverflowCss}}.addr{text-align:left}.note{min-width:80px}.check{font-size:16px;font-weight:700}.fee{font-size:9px;font-weight:700;line-height:1.5}.nowrap{white-space:nowrap}.sign{display:flex;justify-content:space-around;margin-top:36px;font-size:15px;break-inside:avoid;page-break-inside:avoid}.sign-col{text-align:center}.sign-col p{margin:4px 0}.sign-date{text-align:center;margin-top:8px;font-size:14px;break-inside:avoid;page-break-inside:avoid}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class="head">${logo?`<img class="logo" src="${esc(logo)}">`:''}<h1>${esc(e.name||'ค่ายลูกเสือ TERANGGANU 2026')}</h1><p>ใบรายชื่อ${isStudent?'นักเรียน':'ครู'}ผู้เข้าร่วมกิจกรรม · จำนวน ${rows.length} คน</p><p class="mode">${modeLabel}</p></div>${refBlock}<table>${colgroup}<thead><tr>${head}</tr></thead><tbody>${body}</tbody></table>${signBlock}</body></html>`
