...
function openRosterPrint(type,rows,mode='internal'){
  ...
  const colgroup=isStudent?'<colgroup><col style="width:3%"><col style="width:5%"><col style="width:15%"><col style="width:15%"><col style="width:8%"><col style="width:7%"><col style="width:8%"><col style="width:4%"><col style="width:8%"><col style="width:12%"><col style="width:5%"><col style="width:6%"><col style="width:4%"></colgroup>':''
  // ใช้ภายใน: ตัดชื่อยาวด้วย "…" ให้ทุกแถวสูงเท่ากัน สะดวกไล่ตรวจความครบถ้วน
  // ส่งราชการ: ห้ามตัดข้อมูล ปล่อยให้ชื่อตกบรรทัดได้ตามจริง
  ...
  const w=window.open('','_blank');if(!w){toast('เบราว์เซอร์ปิดกั้นหน้าต่างพิมพ์','error');return}w.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>ใบรายชื่อ${isStudent?'นักเรียน':'ครู'} TERANGGANU 2026${isOfficial?' (ส่งราชการ)':''}</title><style>@page{size:A4 landscape;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,'Sarabun',sans-serif;color:#111;margin:0;font-size:14px}.tools{text-align:center;margin-bottom:14px}.tools button{font-size:16px;font-weight:700;padding:10px 18px}.head{text-align:center}.logo{width:88px;height:88px;object-fit:contain;filter:grayscale(1);margin:auto}.head h1{font-size:23px;line-height:1.3;margin:5px}.head p{margin:4px;font-size:16px;font-weight:600}.head .mode{font-size:12px;font-weight:400;color:#555}table{width:100%;border-collapse:collapse;margin-top:16px;font-size:10.5px;line-height:1.25;table-layout:fixed}thead{display:table-header-group}tr{break-inside:avoid;page-break-inside:avoid}th,td{border:1.2px solid #111;padding:5px 4px;text-align:center;height:26px;vertical-align:middle;overflow-wrap:break-word}th{background:#e8e8e8;font-size:10px;font-weight:700}.name{text-align:left;font-weight:600;${nameOverflowCss}}.addr{text-align:left}.note{min-width:80px}.check{font-size:16px;font-weight:700}.fee{font-size:9px;font-weight:700;line-height:1.5}.nowrap{white-space:nowrap}@media print{.tools{display:none}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class="tools"><button onclick="window.print()">🖨️ พิมพ์ / บันทึก PDF</button></div><div class="head">${logo?`<img class="logo" src="${esc(logo)}">`:''}<h1>${esc(e.name||'ค่ายลูกเสือ TERANGGANU 2026')}</h1><p>ใบรายชื่อ${isStudent?'นักเรียน':'ครู'}ผู้เข้าร่วมกิจกรรม · จำนวน ${rows.length} คน</p><p class="mode">${modeLabel}</p></div><table>${colgroup}<thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></body></html>`);w.document.close()
}
