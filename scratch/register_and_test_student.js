...
async function run() {
  try {
    const studentCode = '23392'
    const studentId = 2666
    ...
    const { data: signUpData, error: signUpErr } = await supabase.auth.signUp({
      ...
      password,
      options: {
        data: { full_name: 'น.ส.อัลซาเบีย แซ่ลี', role: 'student', user_code: studentCode }
      }
    })
    ...
    if (linkErr) {
      console.log('Link profile error (might already be linked):', linkErr.message)
    } else {
